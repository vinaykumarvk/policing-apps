...
**Service: Sanction of Building Plans \- Under Self certification Scheme (Private Property)**
...
Document Title: BRD \- Sanction of Building Plans \- Under Self certification Scheme (Private Property)

Owning Department: Town Planning / Building Plan Branch (Assumption)

Primary Source of Truth: PUDA service details page (input) accessed on 02 Feb 2026: https://www.puda.gov.in/service-details-private-properties/1796
...
The 'Sanction of Building Plans \- Under Self certification Scheme (Private Property)' service enables citizens to apply online through the PUDA portal/mobile app, upload required documents, and obtain Sanction Letter / Approved Building Plan after role-based scrutiny and approvals. The workflow supports tracking, queries/rework, and issuance of the final output.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Sanction of Building Plans \- Under Self certification Scheme (Private Property) |
...
| Output | Sanction Letter / Approved Building Plan |
| Physical Verification | Not indicated |
| Fees/Payments | Not specified in input (Open Question). Mining charges receipt may be applicable. |
| Published Disposal Time (SLS) | 7 Day(s) |
| Source URL | https://www.puda.gov.in/service-details-private-properties/1796 |
...
* Application is approved and Sanction Letter / Approved Building Plan is issued to applicant, OR application is rejected with recorded reason.  
...
| :---- | :---- | :---- | :---- |
| Clerk | Scrutiny/verification and forwarding as per stage. | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 1 day(s) |
| Senior Assistant | Scrutiny/verification and forwarding as per stage. | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 1 day(s) |
| Draftsman | Technical scrutiny of building plan drawings and compliance as per scheme (Assumption). | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 2 day(s) |
| SDO | Supervisory scrutiny and approval/rejection. | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 3 day(s) |
...
* Generation and delivery of Sanction Letter / Approved Building Plan via portal/mobile download.
...
* Digitally generated Sanction Letter / Approved Building Plan published to applicant dashboard.
...
| 2 | Clerk | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- Clerk | 1 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 3 | Senior Assistant | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- Senior Assistant | 1 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 4 | Draftsman | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- Draftsman | 2 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 5 | SDO | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- SDO | 3 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 6 | System | Final approval decision | Generate Sanction Letter / Approved Building Plan with unique number; apply digital signature (if enabled); store and publish to applicant dashboard | Sanction Letter / Approved Building Plan issued / Application closed | Approved \- Issued / Rejected | Same day | If rejected, reason is communicated and application is closed. |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Form- A | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-02 | Form- B | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-03 | Form \- C | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-04 | Indemnity Bond | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-05 | Mining Charges Fee Receipt. (Mining Charges to be deposited at Mining Department) (if applicable) | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-06 | Proof of Ownership (Allotment Letter / Change of Ownership) | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-07 | Any other supporting document | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-08 | Sanction of Building Plans DWG file | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-09 | Sanction of Building Plans PDF file | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-10 | Undertaking / Affidavit by Applicant | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-11 | Undertaking for Public Health Component | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-12 | Zoning plan | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
...
| FR-11 | System shall allow competent authority to approve/reject and system shall generate the final certificate/letter/order and publish it to citizen dashboard. | Must | On approval, output is generated with unique number and downloadable; on rejection, reason is shown to citizen. |
| FR-12 | System shall allow internal users to generate/print internal notes and letters (if applicable) and attach them to the case file. | Could | Generated internal notes are stored and visible to authorised roles. |
| FR-13 | System shall maintain complete application history including versions of documents and workflow actions for audit. | Must | History view shows all versions and actions with timestamps. |
| FR-14 | System shall support search and filters for internal users by ARN, property identifiers and applicant name. | Should | Search returns relevant applications; results respect jurisdiction access. |
| FR-15 | System shall provide configurable reason codes for query and rejection to standardise communication. | Should | Internal users can select reason codes; codes appear in notifications and letters. |
| FR-16 | System shall support download/print of blank application form and sample document templates where provided on the portal. | Should | User can download blank form and sample templates from the service page. |
| FR-17 | System shall support building plan files in DWG and PDF formats and maintain versioning across resubmissions. | Must | DWG and PDF are accepted; versions are tracked and accessible to authorised roles. |
| FR-18 | System shall support capturing mining charges fee receipt as an optional document when applicable and allow verifier to mark it verified/unverified. | Should | When uploaded, receipt is displayed and verifiable; if marked required by business rule, submission validation enforces it. |
| FR-19 | System shall provide configurable SLA timers and escalation alerts for each workflow stage. | Should | Stage due dates are computed; breaches are highlighted and alerts are triggered. |
...
| BR-06 | DWG and PDF plan documents must correspond to the same revision; mismatches should trigger query. | Must | Rule is enforced via validations/workflow; evidence is visible in case history/audit. |
| BR-07 | Mining charges receipt is required only when mining charges are applicable; applicability criteria to be confirmed. | Must | Rule is enforced via validations/workflow; evidence is visible in case history/audit. |
...
PUDA: Your 'Sanction of Building Plans \- Under Self certification Scheme (Private Property)' application has been submitted. ARN: {ARN}. Track status on portal.

SMS \- Query Raised (Template)

PUDA: Query raised for ARN {ARN} in 'Sanction of Building Plans \- Under Self certification Scheme (Private Property)'. Please respond by {DUE\_DATE} on portal.

SMS \- Application Approved (Template)

PUDA: Your 'Sanction of Building Plans \- Under Self certification Scheme (Private Property)' application ARN {ARN} is approved. Download certificate/letter from portal.

SMS \- Application Rejected (Template)

PUDA: Your 'Sanction of Building Plans \- Under Self certification Scheme (Private Property)' application ARN {ARN} is rejected. Reason: {SHORT\_REASON}. Details on portal.
...
| TC-06 | Internal scrutiny without inspection | Application submitted | Internal roles verify documents and approve | Application moves through stages as per roles | Workflow |
...
* Mining charges: What is the applicability criteria and how is receipt verified (manual vs integration with mining department)?  
* Building plan scrutiny rules: Are there automated validations (FAR/setbacks) or fully manual scrutiny under self-certification scheme?

## **19.6 References**

* Service details page (input): https://www.puda.gov.in/service-details-private-properties/1796  
