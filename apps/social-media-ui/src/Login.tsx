...
interface LoginProps { onLogin: (username: string, password: string) => Promise<void> }
...
export default function Login({ onLogin }: LoginProps) {
  ...
  return (
    <div className="page">
      <a href="#sm-login-main" className="skip-link">{t("common.skip_to_main")}</a>
      ...
      <main id="sm-login-main" className="panel login-panel" role="main">
        <form onSubmit={handleSubmit} className="login-form">
          {error ? <Alert variant="error">{error}</Alert> : null}
          <Field label={t("login.username")} htmlFor="sm-login-username" required>
            <Input id="sm-login-username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} required placeholder={t("login.username_placeholder")} autoComplete="username" />
          </Field>
          <Field label={t("login.password")} htmlFor="sm-login-password" required>
            <PasswordInput id="sm-login-password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder={t("login.password_placeholder")} autoComplete="current-password" />
          </Field>
          <Button type="submit" disabled={loading} fullWidth>{t(loading ? "login.loading" : "login.submit")}</Button>
          {import.meta.env.DEV && (
