...
import {
  Alert, Badge, Button, Field, Input, Select, Tabs, Textarea, useToast,
} from "@puda/shared";
import { apiBaseUrl, LegalMappingRule } from "../types";
    ...
    <>
              ...
              {rules.map((rule) => (
                <tr key={rule.rule_id}>
                  ...
                  <td data-label={t("legal.status")}>
                    <Badge variant={STATUS_COLORS[rule.approval_status] || "default"}>
      ...
      {formOpen && (
        <div className="modal-backdrop" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div className="panel" style={{ width: "min(100%, 48rem)", maxHeight: "90dvh", overflowY: "auto", padding: "var(--space-4)" }}>
            <h2>{editing ? t("legal.edit_rule") : t("legal.create_rule")}</h2>
            ...
            <h3 style={{ marginTop: "var(--space-4)" }}>{t("legal.expression")}</h3>
            ...
            {formConditions.map((cond, idx) => (
              <div key={idx} style={{ display: "flex", gap: "var(--space-2)", alignItems: "flex-end", marginTop: "var(--space-2)", flexWrap: "wrap" }}>
                ...
                <Field label={t("legal.value")} htmlFor={`v-${idx}`}>
                  <Input id={`v-${idx}`} value={cond.value} onChange={(e) => updateCondition(idx, "value", e.target.value)} placeholder={["in", "not_in", "contains_any", "contains_all"].includes(cond.op) ? "comma-separated" : ""} />
                </Field>
            ...
            <Button size="sm" variant="secondary" onClick={() => setFormConditions((p) => [...p, emptyCondition()])} style={{ marginTop: "var(--space-2)" }}>
  ...
  const testTab = (
    <div style={{ maxWidth: "48rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-3)", marginTop: "var(--space-3)" }}>
        ...
        <Field label={t("legal.test_platform")} htmlFor="tp"><Input id="tp" value={testPlatform} onChange={(e) => setTestPlatform(e.target.value)} placeholder="telegram" /></Field>
