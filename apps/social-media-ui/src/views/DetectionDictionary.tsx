        ...
        <div className="admin-section">
          ...
          <div className="filter-bar" style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap", alignItems: "flex-end", marginBottom: "var(--space-3)" }}>
            ...
            <Field label={t("filter.category")} htmlFor="dd-cat">
              <select id="dd-cat" className="input" value={filterCategory} onChange={(e) => { setFilterCategory(e.target.value); setOffset(0); }}>
                <option value="">{t("filter.all")}</option>
                {facetCategories.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
            </Field>
            <Field label={t("content.language")} htmlFor="dd-lang">
              <select id="dd-lang" className="input" value={filterLanguage} onChange={(e) => { setFilterLanguage(e.target.value); setOffset(0); }}>
                <option value="">{t("filter.all")}</option>
