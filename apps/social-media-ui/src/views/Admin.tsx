...
export default function Admin({ authHeaders, isOffline }: Props) {
  ...
  useEffect(() => { if (isOffline) { setLoading(false); return; } loadUsers(); }, [authHeaders, isOffline]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false); setNewUsername(""); setNewFullName(""); setNewEmail(""); setNewPassword("");
      loadUsers();
  ...
  return (
    <>
      <div className="page__header"><h1>{t("admin.title")}</h1></div>
      {error && <Alert variant="error">{error}</Alert>}
      <div className="admin-section">
        <div className="admin-section__header">
          <h2>{t("admin.users")}</h2>
          <Button onClick={() => setShowForm(!showForm)} disabled={isOffline}>{showForm ? t("common.cancel") : t("admin.create_user")}</Button>
        </div>
        {showForm && (
          <form onSubmit={handleCreate} className="detail-section" style={{ marginBottom: "var(--space-4)" }}>
            <div className="detail-grid">
              <Field label={t("admin.username")} htmlFor="new-username" required><Input id="new-username" value={newUsername} onChange={(e) => setNewUsername(e.target.value)} required /></Field>
              <Field label={t("admin.full_name")} htmlFor="new-fullname" required><Input id="new-fullname" value={newFullName} onChange={(e) => setNewFullName(e.target.value)} required /></Field>
              <Field label={t("admin.email")} htmlFor="new-email"><Input id="new-email" type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} /></Field>
              <Field label={t("login.password")} htmlFor="new-password" required><Input id="new-password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required /></Field>
            </div>
