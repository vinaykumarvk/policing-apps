...
export async function registerTranslateRoutes(app: FastifyInstance): Promise<void> {
  ...
  // POST /api/v1/glossary — Create/upsert glossary term
  ...
  // DELETE /api/v1/glossary/:termId — Soft delete glossary term
  ...
  app.post("/api/v1/translate", {
    ...
    try {
      ...
      // Resolve table, text column, and id column via validated allowlist
      let resolved;
      try {
      ...
      // Translation uses 'summary' for leads (translatable human text)
