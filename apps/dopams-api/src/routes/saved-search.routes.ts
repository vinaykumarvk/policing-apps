...
export async function registerSavedSearchRoutes(app: FastifyInstance): Promise<void> {
  /**
   * GET /api/v1/saved-searches — List saved searches for the authenticated user.
   */
  app.get("/api/v1/saved-searches", {
    ...
    try {
      ...
      return { savedSearches: result.rows.map(({ total_count, ...r }: any) => r), total };
    } catch (err: unknown) {
  ...
  /**
   * GET /api/v1/saved-searches/:id — Retrieve a specific saved search by ID.
   */
  ...
  /**
   * PUT /api/v1/saved-searches/:id — Update an existing saved search.
   */
  ...
  /**
   * DELETE /api/v1/saved-searches/:id — Soft-delete a saved search.
   */
  ...
  /**
   * POST /api/v1/saved-searches/:id/run — Execute the stored query against DOPAMS entities.
   * Supported query fields: alert_type, severity, keywords (array), date_from, date_after.
   */
  app.post("/api/v1/saved-searches/:id/run", {
    ...
    try {
      ...
      // Build parameterized WHERE clause from stored query definition
      const conditions: string[] = [];
      const params: unknown[] = [];
      let idx = 1;

      const alertType = queryDef.alert_type as string | undefined;
      if (alertType) {
        conditions.push(`a.alert_type = $${idx++}`);
        params.push(alertType);
      }

      const severity = queryDef.severity as string | undefined;
      if (severity) {
        conditions.push(`a.severity = $${idx++}`);
        params.push(severity);
      }

      const dateFrom = queryDef.date_from as string | undefined;
      if (dateFrom) {
        conditions.push(`a.created_at >= $${idx++}`);
        params.push(dateFrom);
      }

      const dateAfter = queryDef.date_after as string | undefined;
      if (dateAfter) {
        conditions.push(`a.created_at <= $${idx++}`);
      ...
      if (keywords && keywords.length > 0) {
        const keywordClauses = keywords.map((kw) => {
          params.push(kw);
          const i = idx++;
          return `(a.title ILIKE '%' || $${i} || '%' OR a.description ILIKE '%' || $${i} || '%')`;
        });
      ...
      const total = resultData.rows.length > 0 ? parseInt(resultData.rows[0].total_count, 10) : 0;

      // Update last_run_at and result_count
      await query(
      ...
      return {
        searchId: id,
        results: resultData.rows.map(({ total_count, ...r }: any) => r),
        total,
      };
    } catch (err: unknown) {
