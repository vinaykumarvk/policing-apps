...
export async function registerDashboardRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/v1/reports/scheduled", async (request, reply) => {
    try {
      ...
      request.log.error(err, "Failed to list scheduled reports");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to list scheduled reports" });
    }
  });
  ...
  app.post("/api/v1/reports/scheduled", {
    ...
    try {
      ...
      request.log.error(err, "Failed to create scheduled report");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to create scheduled report" });
    }
  });
  ...
  app.patch("/api/v1/reports/scheduled/:reportId", {
    ...
    try {
      ...
      request.log.error(err, "Failed to update scheduled report");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to update scheduled report" });
    }
  });
