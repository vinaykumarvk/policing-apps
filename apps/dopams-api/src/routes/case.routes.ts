...
export async function registerCaseRoutes(app: FastifyInstance): Promise<void> {
  const requireCaseAction = createRoleGuard(["INTELLIGENCE_ANALYST", "SUPERVISORY_OFFICER", "ADMINISTRATOR"]);
  app.get("/api/v1/cases", {
    ...
    try {
      ...
      request.log.error(err, "Failed to list cases");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to list cases" });
    }
  });

  app.get("/api/v1/cases/facets", async (request, reply) => {
    try {
      const unitId = request.authUser?.unitId || null;
      const [stateRows, priorityRows] = await Promise.all([
        query(`SELECT state_id AS value, COUNT(*)::int AS count FROM dopams_case WHERE ($1::uuid IS NULL OR unit_id = $1::uuid) GROUP BY state_id ORDER BY count DESC`, [unitId]),
        query(`SELECT priority AS value, COUNT(*)::int AS count FROM dopams_case WHERE ($1::uuid IS NULL OR unit_id = $1::uuid) GROUP BY priority ORDER BY count DESC`, [unitId]),
      ]);
      return { facets: { state_id: stateRows.rows, priority: priorityRows.rows } };
    } catch (err) {
      request.log.error(err, "Failed to fetch case facets");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to fetch case facets" });
    }
  });
    ...
    try {
      ...
      const result = await query(
        `SELECT case_id, case_number, title, description, case_type, priority, state_id, row_version,
         ...
         FROM dopams_case WHERE case_id = $1 AND ($2::uuid IS NULL OR unit_id = $2::uuid)`,
        [id, unitId],
      );
      ...
      request.log.error(err, "Failed to fetch case");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to fetch case" });
    }
  });
  ...
  app.post("/api/v1/cases", {
    ...
    if (!requireCaseAction(request, reply)) return;
    try {
      ...
      request.log.error(err, "Failed to create case");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to create case" });
    }
  });
  ...
  app.get("/api/v1/cases/:id/transitions", {
    ...
    try {
      ...
      request.log.error(err, "Failed to fetch case transitions");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to fetch case transitions" });
    }
  });
  ...
  app.post("/api/v1/cases/:id/transition", {
    ...
    try {
      ...
      request.log.error(err, "Failed to transition case");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to transition case" });
    }
  });
}
