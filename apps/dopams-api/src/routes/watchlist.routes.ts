...
export async function registerWatchlistRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/v1/watchlists", {
    ...
    try {
      ...
      request.log.error(err, "Failed to list watchlists");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to list watchlists" });
    }
  });
  ...
  app.get("/api/v1/watchlists/:id", {
    ...
    try {
      ...
      request.log.error(err, "Failed to fetch watchlist");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to fetch watchlist" });
    }
  });
  ...
  app.delete("/api/v1/watchlists/:id/subjects/:subjectId", {
    ...
    try {
      ...
      request.log.error(err, "Failed to remove subject from watchlist");
      reply.code(500).send({ error: "INTERNAL_ERROR", message: "Failed to remove subject from watchlist" });
    }
  });
}
