...
const requireReportAccess = createRoleGuard(["ADMINISTRATOR", "SUPERVISORY_OFFICER", "INTELLIGENCE_ANALYST"]);
...
export async function registerReportTemplateRoutes(app: FastifyInstance): Promise<void> {
  ...
  // 1. Severity Summary — Alerts by severity
  app.get("/api/v1/reports/mis/severity-summary", {
    schema: {
      querystring: {
        type: "object",
        additionalProperties: false,
    ...
    try {
      ...
      const result = await query(
        `SELECT severity,
                ...
                COUNT(*) FILTER (WHERE state_id = 'ACKNOWLEDGED')::int AS acknowledged_count,
                COUNT(*) FILTER (WHERE state_id = 'ESCALATED')::int AS escalated_count,
                COUNT(*) FILTER (WHERE state_id = 'RESOLVED')::int AS resolved_count,
      ...
      request.log.error(err, "Failed to generate severity summary");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });

  // 2. Lead Pipeline — Leads by state
  app.get("/api/v1/reports/mis/lead-pipeline", {
    schema: {
      querystring: {
        type: "object",
        additionalProperties: false,
    ...
    try {
      ...
      const result = await query(
        `SELECT state_id,
                ...
                AVG(EXTRACT(EPOCH FROM (COALESCE(updated_at, NOW()) - created_at)) / 3600)::numeric(10,2) AS avg_hours_in_state
         FROM lead
         WHERE ($1::date IS NULL OR created_at >= $1::date)
           AND ($2::date IS NULL OR created_at <= ($2::date + INTERVAL '1 day'))
         GROUP BY state_id
      ...
      request.log.error(err, "Failed to generate lead pipeline");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });
  ...
  // 4. Category Trends — Alert category trends over time
  ...
  // 5. Officer Workload — Cases per officer
  app.get("/api/v1/reports/mis/officer-workload", {
    schema: {
      querystring: {
        type: "object",
        additionalProperties: false,
    ...
    try {
      ...
      return { officers: result.rows, generatedAt: new Date().toISOString() };
    } catch (err: unknown) {
      request.log.error(err, "Failed to generate officer workload");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });
    ...
    try {
      ...
      const result = await query(
        `SELECT
           ...
           COUNT(*) FILTER (WHERE state_id = 'RESOLVED')::int AS stage_resolved,
           COUNT(*) FILTER (WHERE state_id = 'CLOSED')::int AS stage_closed,
           ROUND(
