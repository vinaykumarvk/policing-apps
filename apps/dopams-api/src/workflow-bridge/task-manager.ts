...
export class DopamsTaskManager implements TaskManager {
  async createTask(
    entityRef: EntityRef,
    stateId: string,
    roleId: string,
    ...
    const taskId = uuidv4();
    await client.query(
      `INSERT INTO task (task_id, entity_type, entity_id, state_id, role_id, status, sla_due_at, created_at)
  ...
  async completeCurrentTask(
    ...
    const client = getClient(txn);
    await client.query(
      `UPDATE task SET status = 'COMPLETED', completed_at = NOW(), decision = $1, remarks = $2
       WHERE entity_id = $3 AND entity_type = $4 AND status IN ('PENDING', 'IN_PROGRESS')`,
