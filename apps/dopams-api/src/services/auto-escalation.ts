...
async function getParentUnit(unitId: string): Promise<{ unit_id: string; unit_name: string; tier_level: string } | null> {
  ...
  try {
    ...
    const breached = await query(
      `SELECT a.alert_id, a.priority, a.unit_id, a.escalation_level,
       ...
       FROM alert a
       JOIN sla_rule sr ON sr.priority = a.priority AND sr.entity_type = 'alert' AND sr.is_active = TRUE
    ...
    logError("Auto-escalation check failed", { error: String(err), stack: err instanceof Error ? err.stack : undefined });
    return 0;
  }
}
...
export async function approveEscalation(
  ...
  const alertResult = await query(
    "SELECT alert_id, unit_id FROM alert WHERE alert_id = $1 AND pending_approval = TRUE",
    [alertId],
  );
