...
const connectionString = process.env.DOPAMS_DATABASE_URL || process.env.DATABASE_URL;
const resolvedConnectionString =
  connectionString || (isTestRuntime ? "postgres://puda:puda@localhost:5432/dopams" : undefined);
if (!resolvedConnectionString && !isTestRuntime) {
  throw new Error("FATAL: DOPAMS_DATABASE_URL must be set in non-test runtime");
}
...
export const pool = new pg.Pool({
  connectionString: resolvedConnectionString || "postgres://puda:puda@localhost:5432/dopams",
