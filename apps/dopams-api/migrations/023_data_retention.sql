...
-- Defines retention policies and automated cleanup for time-bound data.
...
VALUES
  ...
  ('audit_event', 2555, 'created_at', false, 'Audit events — 7 year retention per compliance')
ON CONFLICT (table_name) DO NOTHING;

-- Cleanup function: called by a scheduled job (cron or pg_cron)
...
BEGIN
  FOR pol IN
    ...
    WHERE p.enabled = true
      AND p.table_name != 'audit_event'  -- audit events are immutable
  LOOP
