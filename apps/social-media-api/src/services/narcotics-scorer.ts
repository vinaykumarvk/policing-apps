 ...
 * 2. × Activity multiplier (distribution=3x, active sale=2.5x, purchase=1.5x, use=1x)
...
export async function classifyNarcotics(
  ...
  if (txResult.activityMultiplier > 1) {
    riskFactors.push({
      ...
      detail: `${txResult.activityType} activity (×${txResult.activityMultiplier}). Signals: ${txResult.signals.map(s => s.matched).join(", ")}`,
    });
  }
