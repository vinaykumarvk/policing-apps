...
export async function buildEntityContext(
  ...
  if (entityType === "sm_alert") {
    const alertRes = await query(
      `SELECT a.description, a.content_id,
              ci.platform, ci.language, ci.sentiment, ci.threat_score, ci.content_text
       FROM sm_alert a
    ...
    const classRes = await query(
      `SELECT category, risk_score FROM classification_result
       WHERE entity_type = 'content_item' AND entity_id = (
         SELECT content_id FROM sm_alert WHERE alert_id = $1
