...
function extractEntityInfo(url: string): { entityType: string | null; entityId: string | null } {
  ...
  // Walk segments to find entity type and optional trailing ID
  // Typical pattern: /api/v1/<entity>/:id or /api/v1/<entity>
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // First meaningful segment is the entity type
    if (!entityType) {
      entityType = seg;
      continue;
    }

    // If the next segment looks like a UUID or numeric ID, treat it as entity_id
    if (/^[0-9a-f-]{8,}$/i.test(seg) || /^\d+$/.test(seg)) {
      entityId = seg;
      break;
    }

    // Otherwise it might be a sub-resource — update entity type
    entityType = seg;
  }
