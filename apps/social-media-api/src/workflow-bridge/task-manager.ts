...
export class SmTaskManager implements TaskManager {
  async createTask(entityRef: EntityRef, stateId: string, roleId: string, slaDueAt: Date | null, _metadata: Record<string, unknown>, txn: TransactionHandle): Promise<string> {
    ...
    const taskId = uuidv4();
    await client.query(
      `INSERT INTO case_task (task_id, entity_type, entity_id, state_id, role_id, status, sla_due_at, created_at) VALUES ($1, $2, $3, $4, $5, 'PENDING', $6, NOW())`,
  ...
  async completeCurrentTask(entityRef: EntityRef, actor: Actor, decision: string, remarks: string | undefined, txn: TransactionHandle): Promise<void> {
    const client = getClient(txn);
    await client.query(
      `UPDATE case_task SET status = 'COMPLETED', completed_at = NOW(), decision = $1, remarks = $2 WHERE entity_id = $3 AND entity_type = $4 AND status IN ('PENDING', 'IN_PROGRESS')`,
  ...
  async hasOpenTask(entityRef: EntityRef, stateId: string, txn: TransactionHandle): Promise<boolean> {
    ...
    const result = await client.query(
      `SELECT 1 FROM case_task WHERE entity_id = $1 AND entity_type = $2 AND state_id = $3 AND status IN ('PENDING', 'IN_PROGRESS') LIMIT 1`,
