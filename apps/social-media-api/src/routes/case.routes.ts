...
export async function registerCaseRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/v1/cases", async () => {
    const result = await query(
      `SELECT case_id, case_number, title, priority, state_id,
              source_alert_id, assigned_to, created_by, created_at
       FROM case_record ORDER BY created_at DESC`,
    );
  ...
  app.post("/api/v1/cases", {
    ...
    const result = await query(
      `INSERT INTO case_record (title, description, source_alert_id, created_by) VALUES ($1, $2, $3, $4)
       RETURNING case_id, case_number, title, description, priority, state_id, source_alert_id, created_by, created_at`,
      [title, description || null, alertId || null, userId],
    );
    reply.code(201);
  ...
  app.get("/api/v1/cases/:id", {
    ...
    const result = await query(
      `SELECT case_id, case_number, title, description, priority, state_id, row_version,
