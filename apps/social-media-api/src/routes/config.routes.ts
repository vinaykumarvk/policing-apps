...
export async function registerConfigRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/v1/config/workflows", async (_request, reply) => {
    try {
      ...
      _request.log.error(err, "Failed to list workflow definitions");
  ...
  app.get("/api/v1/config/roles", async (_request, reply) => {
    try {
      ...
      _request.log.error(err, "Failed to list roles");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });

  // GET /api/v1/config/taxonomy — list taxonomy categories (SM-specific)
  app.get("/api/v1/config/taxonomy", async (_request, reply) => {
    try {
      const result = await query(
        `SELECT category_id, name, description, is_active, created_at
         FROM taxonomy_category ORDER BY name ASC`,
      );
      return { categories: result.rows, total: result.rows.length };
    } catch (err: unknown) {
      _request.log.error(err, "Failed to list taxonomy categories");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });

  // GET /api/v1/config/sla — list SLA configuration per workflow state
  app.get("/api/v1/config/sla", async (_request, reply) => {
    try {
      const defDir = path.resolve(__dirname, "..", "workflow-definitions");
      const files = fs.readdirSync(defDir).filter((f) => f.endsWith(".json"));
      ...
      _request.log.error(err, "Failed to list SLA configuration");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });
}
