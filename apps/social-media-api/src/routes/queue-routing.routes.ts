...
/**
 * FR-07 AC-05: Queue routing rules CRUD.
 * Rules map classification category + risk score ranges to target queues.
 */
export async function registerQueueRoutingRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/v1/queue-routing/rules", async (request, reply) => {
    try {
  ...
  // POST /api/v1/queue-routing/route — Route a content item to the appropriate queue
  app.post("/api/v1/queue-routing/route", {
    ...
    try {
      ...
      // Get the content's classification
      const classResult = await query(
        `SELECT category, risk_score FROM classification_result WHERE entity_type = 'sm_content' AND entity_id = $1
         ORDER BY updated_at DESC LIMIT 1`,
        [contentId],
      );

      if (classResult.rows.length === 0) {
        return send404(reply, "CLASSIFICATION_NOT_FOUND", "No classification found for this content item");
      }

      const { category, risk_score } = classResult.rows[0];
      const riskScore = parseFloat(risk_score);

      // Find matching queue routing rule (highest priority_order first)
      ...
      // Update content item queue
      await query(`UPDATE content_item SET queue_name = $1 WHERE content_id = $2`, [targetQueue, contentId]);

      return { contentId, targetQueue, category, riskScore };
    } catch (err: unknown) {
      request.log.error(err, "Failed to route content");
      return sendError(reply, 500, "INTERNAL_ERROR", "An internal error occurred");
    }
  });
}
