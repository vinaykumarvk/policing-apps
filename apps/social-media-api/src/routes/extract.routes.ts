...
export async function registerExtractRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/extract/:entityType/:entityId", {
    ...
    try {
      ...
      switch (entityType) {
        case "sm_alert":
          sql = `SELECT description AS text FROM sm_alert WHERE alert_id = $1`;
          break;
        case "sm_case":
          sql = `SELECT description AS text FROM case_record WHERE case_id = $1`;
          break;
        case "sm_evidence":
          // evidence_item has no description; pull text from linked content_item
          sql = `SELECT COALESCE(c.content_text, '') AS text
                 FROM evidence_item e
                 LEFT JOIN content_item c ON c.content_id = e.content_id
