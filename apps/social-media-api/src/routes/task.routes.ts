...
export async function registerTaskRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/v1/tasks/inbox", {
    ...
    const countResult = await query(
      `SELECT COUNT(*) AS total FROM case_task WHERE status IN ('PENDING', 'IN_PROGRESS') AND role_id = ANY($1)`,
      [roles],
    );
  ...
  app.post("/api/v1/tasks/:id/action", {
    ...
    const taskResult = await query(
      `SELECT task_id, entity_type, entity_id, state_id, role_id, assigned_to FROM case_task WHERE task_id = $1 AND status IN ('PENDING', 'IN_PROGRESS')`,
      [id],
    );
