...
import { invalidateSlangCache } from "../services/slang-normalizer";
...
export async function registerSlangRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/v1/slang", {
    ...
    const sortCol = SORT_COLUMNS[qs.sortBy || ""] || "category";
    const sortDir = qs.sortOrder === "desc" ? "DESC" : "ASC";
  ...
  // GET /api/v1/slang/facets — Distinct categories, languages, and term types for filter dropdowns
