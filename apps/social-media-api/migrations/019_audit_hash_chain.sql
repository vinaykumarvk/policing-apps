-- 019_audit_hash_chain.sql
-- Tamper-evident hash chain for audit_log rows and immutability rules.
...
CREATE INDEX IF NOT EXISTS idx_audit_log_chain_order
  ON audit_log(chain_position);
CREATE UNIQUE INDEX IF NOT EXISTS idx_audit_log_hash_unique
  ON audit_log(event_hash) WHERE event_hash IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_audit_log_chain_position_unique
  ON audit_log(chain_position) WHERE chain_position IS NOT NULL;

-- ── Trigger: compute hash on every INSERT ───────────────────────────────────

CREATE OR REPLACE FUNCTION set_audit_log_hash_chain()
RETURNS trigger AS $$
DECLARE
  previous_hash    TEXT;
...
FOR EACH ROW
EXECUTE FUNCTION set_audit_log_hash_chain();
...
SELECT setval(
  'audit_chain_seq',
  COALESCE((SELECT MAX(chain_position) FROM audit_log), 1),
  true
);

-- ── Immutability rules ──────────────────────────────────────────────────────
-- Prevent UPDATE and DELETE on audit_log — the chain must be append-only.
