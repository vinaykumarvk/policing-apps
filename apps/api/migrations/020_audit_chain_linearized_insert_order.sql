...
-- Fix concurrent insert ordering so audit hash chain remains linear and verifiable.

ALTER TABLE audit_event
  ALTER COLUMN chain_position DROP DEFAULT;
...
BEGIN
  ...
  -- Assign chain position only under lock so ordering matches committed insertion order.
...
DECLARE
  ...
  computed_hash TEXT;
BEGIN
  -- Rebuild full chain in current chain_position order.
