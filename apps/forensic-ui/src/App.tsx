/**
 * Forensic Lab UI — Main App shell with sidebar nav, bottom nav, and hash routing.
 */
...
const ImportList = lazy(() => import("./views/ImportList"));
const ImportDetail = lazy(() => import("./views/ImportDetail"));
const FindingDetail = lazy(() => import("./views/FindingDetail"));
...
type View =
  ...
  | "evidence-detail" | "imports" | "import-detail"
  | "finding-detail" | "report-detail"
  | "inbox" | "query-assistant" | "network-graph" | "model-admin"
  | "settings" | "admin"
  | "audit-log";

const VALID_VIEWS = [
  "", "dashboard", "cases", "evidence", "imports", "findings", "reports", "inbox",
...
const NAV_ITEMS: { view: View; key: string; icon: string }[] = [
  ...
  { view: "imports", key: "nav.imports", icon: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12" },
...
export default function App() {
  ...
  const viewToHash = useCallback((): string => {
    ...
    if (view === "import-detail" && resourceId) return buildHash("imports", resourceId);
    if (view === "finding-detail" && resourceId) return buildHash("findings", resourceId);
    ...
    if (view === "imports") return buildHash("imports");
  ...
  useEffect(() => {
    ...
    if (parsed.resourceId) {
      const dm: Record<string, View> = { cases: "case-detail", evidence: "evidence-detail", imports: "import-detail", findings: "finding-detail", reports: "report-detail" };
      if (dm[v]) { setView(dm[v]); setResourceId(parsed.resourceId); return; }
    }
    const sm: Record<string, View> = { dashboard: "dashboard", cases: "cases", imports: "imports", inbox: "inbox", "query-assistant": "query-assistant", "network-graph": "network-graph", "model-admin": "model-admin", settings: "settings", admin: "admin", "audit-log": "audit-log" };
  ...
  const renderNavContent = (context: "sidebar" | "drawer") => (
    <>
      ...
      <ul className="sidebar__nav">
        {NAV_ITEMS.map((item) => (
          <li key={item.view}>
            <button className={`sidebar__item ${view === item.view || (item.view === "cases" && view === "case-detail") ? "sidebar__item--active" : ""}`} onClick={() => navigate(item.view)} title={t(item.key)} type="button">
  ...
  return (
    <>
      <a href="#forensic-main" className="skip-link">{t("common.skip_to_main")}</a>
        ...
        <div className="app-layout__main">
          ...
          <main id="forensic-main" role="main">
            <Suspense fallback={suspenseFallback}>
              ...
              {view === "imports" && <ImportList authHeaders={authHeaders} isOffline={isOffline} onSelect={(id) => navigate("import-detail", id)} />}
              {view === "import-detail" && resourceId && <ImportDetail id={resourceId} authHeaders={authHeaders} isOffline={isOffline} onBack={() => navigate("imports")} />}
              {view === "finding-detail" && resourceId && <FindingDetail id={resourceId} authHeaders={authHeaders} isOffline={isOffline} onBack={() => window.history.back()} />}
