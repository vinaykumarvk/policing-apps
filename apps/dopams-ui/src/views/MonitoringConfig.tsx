...
type MonitoringProfile = {
  profile_id: string; platform: string; entry_type: string; handle?: string;
  url?: string; priority: string; source: string; source_ref?: string;
  suspect_name?: string; notes?: string; last_scraped_at?: string;
};

type JurisdictionLocation = {
  location_id: string; district_name: string; city_names?: string[];
  area_names?: string[]; alt_spellings?: string[]; notes?: string;
};

// DOPAMS-specific sources (not social media platforms)
...
export default function MonitoringConfig({ authHeaders, isOffline, isAdmin }: Props) {
  ...
  const [fpPlatform, setFpPlatform] = useState<string>("cctns");
  ...
  // CSV Import -- uses DOPAMS cookie-based auth
  const handleCsvImport = async (file: File) => {
    setImporting(true);
    try {
      const text = await file.text();
      const token = localStorage.getItem("dopams_auth_token");
  ...
  return (
    <>
        ...
        <div className="admin-section">
          <div className="admin-section__header">
            ...
            {isAdmin && (
              <div style={{ display: "flex", gap: "var(--space-2)", flexWrap: "wrap" }}>
                <input ref={csvInputRef} type="file" accept=".csv" style={{ display: "none" }} onChange={(e) => { const f = e.target.files?.[0]; if (f) handleCsvImport(f); }} />
          ...
          <div className="filter-bar">
            ...
            <Field label={t("monitoring.platform")} htmlFor="mp-platform">
              <Select id="mp-platform" value={filterPlatform} onChange={(e) => { setFilterPlatform(e.target.value); setProfileOffset(0); }}>
                <option value="">{t("filter.all")}</option>
                {PLATFORMS.map((p) => <option key={p} value={p}>{p}</option>)}
              </Select>
            </Field>
            <Field label={t("monitoring.entry_type")} htmlFor="mp-type">
              <Select id="mp-type" value={filterEntryType} onChange={(e) => { setFilterEntryType(e.target.value); setProfileOffset(0); }}>
                <option value="">{t("filter.all")}</option>
                {ENTRY_TYPES.map((et) => <option key={et} value={et}>{et}</option>)}
              </Select>
            </Field>
            <Field label={t("monitoring.source")} htmlFor="mp-source">
              <Select id="mp-source" value={filterSource} onChange={(e) => { setFilterSource(e.target.value); setProfileOffset(0); }}>
                <option value="">{t("filter.all")}</option>
      ...
      {/* EDIT PROFILE MODAL */}
      <Modal open={!!editProfile} title={t("monitoring.edit_profile")} onClose={() => setEditProfile(null)} actions={<><Button onClick={() => setEditProfile(null)}>{t("common.cancel")}</Button><Button onClick={handleSaveProfile} disabled={saving}>{saving ? t("common.loading") : t("common.save")}</Button></>}>
        {editProfile && (
          <div style={{ display: "grid", gap: "var(--space-3)" }}>
            ...
            <Field label={t("monitoring.handle")} htmlFor="ep-handle"><Input id="ep-handle" value={epHandle} onChange={(e) => setEpHandle(e.target.value)} /></Field>
            <Field label={t("monitoring.url")} htmlFor="ep-url"><Input id="ep-url" value={epUrl} onChange={(e) => setEpUrl(e.target.value)} /></Field>
            ...
            <Field label={t("monitoring.source_ref")} htmlFor="ep-sref"><Input id="ep-sref" value={epSourceRef} onChange={(e) => setEpSourceRef(e.target.value)} /></Field>
            <Field label={t("monitoring.suspect_name")} htmlFor="ep-suspect"><Input id="ep-suspect" value={epSuspectName} onChange={(e) => setEpSuspectName(e.target.value)} /></Field>
            <Field label={t("monitoring.notes")} htmlFor="ep-notes"><Input id="ep-notes" value={epNotes} onChange={(e) => setEpNotes(e.target.value)} /></Field>
          </div>
        )}
      </Modal>

      {/* EDIT LOCATION MODAL */}
      <Modal open={!!editLocation} title={t("monitoring.edit_location")} onClose={() => setEditLocation(null)} actions={<><Button onClick={() => setEditLocation(null)}>{t("common.cancel")}</Button><Button onClick={handleSaveLocation} disabled={savingLoc}>{savingLoc ? t("common.loading") : t("common.save")}</Button></>}>
        {editLocation && (
          <div style={{ display: "grid", gap: "var(--space-3)" }}>
            <Field label={t("monitoring.district_name")} htmlFor="el-dist"><Input id="el-dist" value={elDistrict} onChange={(e) => setElDistrict(e.target.value)} /></Field>
            <Field label={t("monitoring.city_names")} htmlFor="el-cities"><Input id="el-cities" value={elCities} onChange={(e) => setElCities(e.target.value)} placeholder={t("monitoring.comma_separated")} /></Field>
            <Field label={t("monitoring.area_names")} htmlFor="el-areas"><Input id="el-areas" value={elAreas} onChange={(e) => setElAreas(e.target.value)} placeholder={t("monitoring.comma_separated")} /></Field>
            <Field label={t("monitoring.alt_spellings")} htmlFor="el-alt"><Input id="el-alt" value={elAltSpellings} onChange={(e) => setElAltSpellings(e.target.value)} placeholder={t("monitoring.comma_separated")} /></Field>
            <Field label={t("monitoring.notes")} htmlFor="el-notes"><Input id="el-notes" value={elNotes} onChange={(e) => setElNotes(e.target.value)} /></Field>
          </div>
        )}
      </Modal>

      {/* DELETE CONFIRMATION MODAL */}
      <Modal open={!!deleteId} title={t("monitoring.confirm_delete")} onClose={() => setDeleteId(null)} actions={<><Button onClick={() => setDeleteId(null)}>{t("common.cancel")}</Button><Button onClick={handleDelete} disabled={deleting}>{deleting ? t("common.loading") : t("slang.delete")}</Button></>}>
