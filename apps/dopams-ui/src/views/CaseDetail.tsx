...
import { Alert, Badge, Button, Field, Input, Select, Tabs, Textarea, useToast } from "@puda/shared";
import { apiBaseUrl, DopamsCase } from "../types";
...
export default function CaseDetail({ id, authHeaders, isOffline, onBack }: Props) {
  ...
  const [caseData, setCaseData] = useState<DopamsCase | null>(null);
        ...
        { key: "details", label: t("detail.tab_details"), content: (
          <>
            <div className="detail-section">
              <h2 className="detail-section__title">{t("case.information_title")}</h2>
              <div className="detail-grid">
                <div className="detail-field">
                  <span className="detail-field__label">{t("cases.case_number")}</span>
                  <span className="detail-field__value">{caseData.case_number}</span>
                </div>
                <div className="detail-field">
                  <span className="detail-field__label">{t("cases.type")}</span>
                  <span className="detail-field__value">{caseData.case_type}</span>
                </div>
                <div className="detail-field">
                  <span className="detail-field__label">{t("cases.priority")}</span>
                  <span className="detail-field__value">{caseData.priority}</span>
                </div>
                <div className="detail-field">
                  <span className="detail-field__label">{t("cases.assigned")}</span>
                  <span className="detail-field__value">{caseData.assigned_to || "—"}</span>
                </div>
                <div className="detail-field">
                  <span className="detail-field__label">{t("detail.created_at")}</span>
                  <span className="detail-field__value">{caseData.created_at ? new Date(caseData.created_at).toLocaleString() : "—"}</span>
                </div>
              </div>
            </div>
