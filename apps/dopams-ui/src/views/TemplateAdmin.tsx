...
import { Card, Button, Alert, Badge, Field, Input, Select, Textarea, SkeletonBlock, useToast } from "@puda/shared";
      ...
      {tab === "editor" && (
        <Card>
          ...
          <div className="detail-grid" style={{ marginBottom: "var(--space-4)" }}>
            <Field label={t("templates.name")} htmlFor="tmpl-name">
              <Input id="tmpl-name" value={name} onChange={(e) => setName(e.target.value)} />
            </Field>
            <Field label={t("templates.type")} htmlFor="tmpl-type">
              <Select id="tmpl-type" value={templateType} onChange={(e) => setTemplateType(e.target.value)}>
                <option value="COURT_EXPORT">{t("templates.court_export")}</option>
                <option value="INTELLIGENCE">{t("templates.intelligence")}</option>
              </Select>
            </Field>
            <Field label={t("templates.active")} htmlFor="tmpl-active">
              <label htmlFor="tmpl-active" style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", cursor: "pointer" }}>
                <input id="tmpl-active" type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} />
            ...
            {sections.map((sec, idx) => (
              <Card key={idx} style={{ marginBottom: "var(--space-2)", padding: "var(--space-3)" }}>
                <div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center", marginBottom: "var(--space-2)", flexWrap: "wrap" }}>
                  <Select value={sec.type} onChange={(e) => updateSection(idx, "type", e.target.value)} style={{ width: "auto" }}>
                  ...
                  <Input
                    ...
                    style={{ flex: 1 }}
                  />
                  <Button size="sm" variant="secondary" onClick={() => moveSection(idx, -1)} disabled={idx === 0} aria-label={t("templates.move_up")}>&#x25B2;</Button>
                  <Button size="sm" variant="secondary" onClick={() => moveSection(idx, 1)} disabled={idx === sections.length - 1} aria-label={t("templates.move_down")}>&#x25BC;</Button>
          ...
          <div style={{ marginBottom: "var(--space-4)" }}>
            ...
            {variables.map((v, idx) => (
              <div key={idx} style={{ display: "flex", gap: "var(--space-2)", alignItems: "center", marginBottom: "var(--space-2)", flexWrap: "wrap" }}>
                <Input
                ...
                />
                <Select value={v.type} onChange={(e) => updateVariable(idx, "type", e.target.value)} style={{ width: "auto" }}>
                  <option value="string">{t("templates.var_type_string")}</option>
                  <option value="date">{t("templates.var_type_date")}</option>
                  <option value="number">{t("templates.var_type_number")}</option>
                </Select>
                <label style={{ display: "flex", alignItems: "center", gap: "var(--space-1)", whiteSpace: "nowrap", minHeight: "2.75rem" }}>
          ...
          {sections.length > 0 && (
            <div style={{ marginBottom: "var(--space-4)" }}>
              ...
              <Card style={{ padding: "var(--space-4)", backgroundColor: "var(--color-surface-subtle)" }}>
                <h2 style={{ marginBottom: "var(--space-2)" }}>{name || t("templates.untitled")}</h2>
                {sections.map((sec, idx) => (
