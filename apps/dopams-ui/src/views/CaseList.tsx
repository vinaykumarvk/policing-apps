...
export default function CaseList({ authHeaders, isOffline, onSelect }: Props) {
  ...
  const [cases, setCases] = useState<DopamsCase[]>([]);
  ...
  useEffect(() => {
    if (isOffline) return;
    fetch(`${apiBaseUrl}/api/v1/cases/facets`, authHeaders())
      ...
      <div className="filter-bar">
        <Field label={t("filter.state")} htmlFor="filter-state">
          <Select id="filter-state" value={stateFilter} onChange={(e) => { setStateFilter(e.target.value); setPage(1); }}>
            ...
            {facetOptions(facets.state_id, ["OPEN", "UNDER_INVESTIGATION", "AWAITING_REVIEW", "CLOSED", "REOPENED"])}
          </Select>
        </Field>
