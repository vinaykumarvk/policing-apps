...
export default function FunnelChart({ stages }: Props) {
  if (!stages.length) {
    return <div className="empty-state"><h3>No data</h3></div>;
  }
  const maxCount = Math.max(1, ...stages.map((s) => s.count));
  return (
    <div className="funnel-chart">
      {stages.map((s, i) => {
        const hours = typeof s.avgHours === "string" ? parseFloat(s.avgHours) : s.avgHours;
        return (
          <div className="funnel-chart__stage" key={i}>
            ...
            <div className="funnel-chart__meta">
              ...
              {hours != null && !isNaN(hours) && (
                <span className="funnel-chart__hours">{hours.toFixed(1)}h avg</span>
              )}
            </div>
          </div>
        );
