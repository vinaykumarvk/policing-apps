...
/** Verifies a platform launch token (HMAC-SHA256, audience "forensic"). */
function verifyPlatformSsoToken(token: string, secret: string): PlatformSsoPayload | null {
  ...
  if (parsed.a !== "forensic" || typeof parsed.e !== "number" || parsed.e <= Date.now()) return null;
...
export async function registerAuthRoutes(app: FastifyInstance): Promise<void> {
  ...
  // Forensic session (cookie-based). Maps the platform subject to a local
