...
async function main() {
  ...
  const port = Number(process.env.PORT || process.env.FORENSIC_API_PORT || 3003);
  const host = process.env.FORENSIC_API_HOST || "0.0.0.0";
  ...
  try {
    ...
    app.log.info(`Forensic API listening on ${host}:${port}`);
