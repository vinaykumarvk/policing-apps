...
const KEYWORD_CATEGORIES: Record<string, string[]> = {
  MALWARE: ["malware", "virus", "trojan", "worm", "rootkit", "backdoor", "keylogger"],
  DATA_EXFILTRATION: ["exfiltrat", "data theft", "leak", "breach", "unauthorized access", "stolen data"],
  NETWORK_INTRUSION: ["intrusion", "penetrat", "exploit", "vulnerab", "zero-day", "brute force"],
  ENCRYPTION: ["encrypt", "decrypt", "cipher", "obfuscat", "steganograph"],
  FINANCIAL_FRAUD: ["fraud", "embezzl", "forgery", "counterfeit", "money laundering", "scam"],
  IDENTITY_THEFT: ["identity theft", "impersonat", "credential", "phishing", "social engineer"],
  CHILD_EXPLOITATION: ["child", "minor", "csam", "underage"],
  COMMUNICATION_ANALYSIS: ["chat", "message", "email", "voip", "call record", "sms"],
  DEVICE_TAMPERING: ["tamper", "wiped", "factory reset", "anti-forensic", "destroyed"],
};
...
export async function classifyEntity(entityType: string, entityId: string): Promise<DbRow> {
  ...
  switch (entityType) {
    case "forensic_case": tableName = "forensic_case"; textColumn = "description"; idColumn = "case_id"; break;
    case "forensic_finding": tableName = "ai_finding"; textColumn = "description"; idColumn = "finding_id"; break;
