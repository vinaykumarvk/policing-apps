...
/**
 * Forensic app uses the existing extracted_entity table from 001_init.sql.
 * PK = entity_id (not extracted_id). Columns source_entity_type, source_entity_id,
 * and normalized_value were added by migration 010.
 */
...
export async function extractAndStore(
  ...
  // Forensic table uses entity_id as PK
  for (const entity of stored) {
    const matches = await query(
      `SELECT entity_id, source_entity_type, source_entity_id FROM extracted_entity
       WHERE entity_type = $1 AND normalized_value = $2 AND entity_id != $3`,
      [entity.entity_type, entity.normalized_value, entity.entity_id],
    );
...
export async function getEntityGraph(entityId: string, depth: number = 2): Promise<{ nodes: DbRow[]; edges: DbRow[] }> {
  ...
  async function traverse(currentId: string, currentDepth: number) {
    ...
    // Forensic table uses entity_id as PK
    const entity = await query(`SELECT * FROM extracted_entity WHERE entity_id = $1`, [currentId]);
    ...
    const rels = await query(
      `SELECT r.*, e1.entity_type as from_type, e1.entity_value as from_value,
       ...
       JOIN extracted_entity e1 ON e1.entity_id = r.from_entity_id
       JOIN extracted_entity e2 ON e2.entity_id = r.to_entity_id
