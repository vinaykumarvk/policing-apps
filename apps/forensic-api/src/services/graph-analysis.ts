...
export async function analyzeNetwork(): Promise<GraphAnalysis> {
  ...
  // Get entity details — forensic app uses entity_id as PK (not extracted_id)
  const entityIds = Array.from(adj.keys());
  const entityResult = await query(
    `SELECT entity_id::text as id, entity_type, entity_value, normalized_value FROM extracted_entity WHERE entity_id = ANY($1::uuid[])`,
    [entityIds]
  );
...
export async function getKingpins(): Promise<DbRow[]> {
  // Forensic app uses entity_id as PK (not extracted_id)
  const result = await query(
    `SELECT gar.*, ee.entity_type as extracted_type, ee.entity_value
     FROM graph_analysis_result gar
     LEFT JOIN extracted_entity ee ON ee.entity_id = gar.entity_id
