...
Service: Issue of No Due Certificate
...
Document Title: BRD \- Issue of No Due Certificate

Owning Department: Accounts Branch (Assumption \- confirm with PUDA)

Primary Source of Truth: PUDA portal 'Issue of No Due Certificate' service details (Service ID 23), accessed 02 Feb 2026\.
...
The 'Issue of No Due Certificate' service enables a citizen/applicant to apply online for issuance of No Due Certificate (NDC) related outputs for a property managed by the authority. The digital workflow supports role-based scrutiny, document submission and verification, time-bound processing as per published Service Level Standards (SLS), and transparent status tracking for the applicant.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Issue of No Due Certificate |
| Service Category | Property/Plot Services |
| Authorities | PUDA, GMADA, GLADA, BDA (as displayed on portal header) |
| Portal Service URL | https://www.puda.gov.in/service-details/23 |
| Application Disposal Time (SLS) | 5 Days |
| Workflow Roles | Submit Application \-\> Clerk (\~1 Day) \-\> Sr. Assistant (Accounts) (\~3 Days) \-\> Account Officer (\~1 Day) \-\> Disposal |
| Document(s) Required | Upload payment receipt in case payment details of transaction is not updated in system |
| Physical Verification | Not explicitly stated on the service page; internal verification of receipt/ledger is expected (Assumption). |
| Output | No Due Certificate issued (PDF) on approval; rejection order on rejection (format/signing to be confirmed). |
...
Request by applicant for No Due Certificate (NDC) related service for a property in authority records.
...
\- All dues for the property are cleared OR applicant has proof of payment to reconcile (Assumption).

## **3.3 Postconditions**

\- On approval, No Due Certificate (NDC) output is issued and made available for applicant download (Assumption \- format/signing to be confirmed).  
...
| :---- | :---- | :---- | :---- |
| Clerk | Initial scrutiny; verify request and property identifiers; forward to Accounts scrutiny or raise query/reject (assumed). | Internal portal dashboard | 1 Days |
| Sr. Assistant (Accounts) | Verify ledger/dues status; validate payment details; review receipt if required; forward to Account Officer or raise query (assumed). | Internal portal dashboard | 3 Days |
| Account Officer | Final approval/rejection for issuance of No Due Certificate; record remarks. | Internal portal dashboard | 1 Days |
...
\- Citizen-facing online application form for Issue of No Due Certificate including required fields and document upload.  
...
| Ledger | Accounts record of dues and receipts for a property (integration to be confirmed). |
...
The digital process will allow online submission and tracking, automated task routing to internal roles, document management, verification/inspection capture (where applicable), and disposal within 5 days as published. The workflow will support structured decisions (forward/query/approve/reject) and time-bound processing.

# **9\. Workflow Details**

The published processing sequence is: Submit Application \-\> Clerk (\~1 Day) \-\> Sr. Assistant (Accounts) (\~3 Days) \-\> Account Officer (\~1 Day) \-\> Disposal.
...
| 3 | Clerk | Application \+ uploaded documents | Initial scrutiny; verify request and property identifiers; forward to Accounts scrutiny or raise query/reject (assumed). | Forwarded / Query raised / Rejected | Pending at Sr. Assistant (Accounts) OR Query Pending OR Rejected | 1 Days | Not explicitly stated on the service page; internal verification of receipt/ledger is expected (Assumption). |
...
| 5 | Sr. Assistant (Accounts) | Application package \+ prior stage remarks | Verify ledger/dues status; validate payment details; review receipt if required; forward to Account Officer or raise query (assumed). | Forwarded / Query raised / Rejected | Pending at Account Officer OR Query Pending OR Rejected | 3 Days | If query is raised, application returns after resubmission (configurable). |
| 6 | Account Officer | Application package \+ verification records | Final approval/rejection for issuance of No Due Certificate; record remarks. Approve or reject with mandatory remarks. | Approved / Rejected | Approved OR Rejected | 1 Days | Final approval may be contingent on completion of physical verification/inspection (as applicable). |
| 7 | System | Final decision | Generate No Due Certificate (NDC) output on approval and/or rejection order; close application; notify applicant. | Output generated; application closed. | Closed | Same day | Digital signing/QR and output format to be confirmed; treated as recommendation if implemented. |
...
| Pending at Clerk | Application pending with Clerk for scrutiny. | System assigns task | Clerk forwards/query/reject | Pending at Clerk \-\> Query Pending / Pending at Sr. Assistant (Accounts) / Rejected |
...
| Pending at Sr. Assistant (Accounts) | Application pending with Sr. Assistant (Accounts) for review. | Forwarded to Sr. Assistant (Accounts) | Sr. Assistant (Accounts) forwards/query/reject | Pending at Sr. Assistant (Accounts) \-\> Query Pending / Pending at Account Officer / Rejected |
| Pending at Account Officer | Application pending with Account Officer for final decision. | Forwarded to Account Officer | Account Officer approves/rejects | Pending at Account Officer \-\> Approved / Rejected |
...
The system shall maintain the following key entities for the Issue of No Due Certificate service:
...
| Application Form | Document Upload | DOC-01: Upload payment receipt in case payment details of transaction is not updated in system | Citizen uploads; internal can mark verified and add remarks |
...
| Submission | DOC-01 payment receipt | Required only if payment details are not updated in the system. | As per service page. |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Upload payment receipt in case payment details of transaction is not updated in system | Applicant | Conditional | Submission | System should require receipt only when payment transaction is missing/not updated (rule). |
...
| ID | Description | Priority | Acceptance Criteria |
| :---- | :---- | :---- | :---- |
| FR-01 | System shall allow citizens to select the Authority (PUDA/GMADA/GLADA/BDA) and initiate the 'Issue of No Due Certificate' service. | Must | Citizen can initiate the service for a selected authority. |
...
| FR-03 | System shall allow upload of required documents (DOC-01) and store them securely with metadata and versioning. | Must | Upload succeeds for allowed formats; documents are retrievable by authorised users. |
...
| FR-07 | System shall provide role-based internal dashboards for Clerk, Sr. Assistant (Accounts), Account Officer to view and process assigned cases. | Must | Internal users see only applications assigned to their role/queue as per RBAC. |
...
| FR-09 | 'Sr. Assistant (Accounts)' role user shall be able to review the application, record scrutiny remarks, and take action: Forward to next stage, Raise Query to Applicant, or Reject (Query/Reject are assumptions unless confirmed). | Should | Role user can perform actions and system changes status accordingly; actions are logged. |
| FR-10 | 'Account Officer' role user shall be able to review the application, record scrutiny remarks, and take action: Forward to next stage, Raise Query to Applicant, or Reject (Query/Reject are assumptions unless confirmed). | Must | Role user can perform actions and system changes status accordingly; actions are logged. |
| FR-11 | System shall support capturing verification/inspection completion status and remarks (including physical verification where required) before allowing final approval. | Must | Final approving role cannot approve unless required verification/inspection statuses are completed (configurable). |
| FR-12 | On approval, system shall generate the No Due Certificate (NDC) output document and make it available for applicant download (Assumption). | Should | Downloadable output is generated and accessible to applicant and authorised staff. |
| FR-13 | On rejection, system shall generate a rejection order with reason and make it available to the applicant. | Should | Rejection order is downloadable and includes reason code/text. |
| FR-14 | System shall provide configurable SLA timers per stage (as per published durations) and calculate overall service disposal time (5 days). | Must | SLA clocks and breach flags are visible in dashboards and reports. |
| FR-15 | System shall send notifications (SMS/email/in-app) on submission, query, resubmission, approval, rejection, and SLA breach (internal). | Should | Notifications are triggered for each configured event and logged. |
| FR-16 | System shall allow authorised staff to search applications by ARN/UPN/Plot/Scheme and export lists to CSV/Excel. | Should | Search results are accurate and exportable. |
| FR-17 | System shall support assisted submission by authorised operators (e.g., Sewa Kendra) with an auditable marker of assisted channel (Assumption). | Could | Operator can submit on behalf of citizen and audit log captures assisted channel. |
...
| :---- | :---- | :---- | :---- |
| BR-01 | Overall service disposal time shall be 5 days (as published on service details page). | Must | System SLA calculation shows total processing within published SLS for non-query cases. |
| BR-02 | Stage-wise SLS shall be as published: Clerk 1 days, Sr. Assistant (Accounts) 3 days, Account Officer 1 days. | Must | SLA timer per stage is configured to published values and breach is flagged. |
| BR-03 | Document submission rules shall be enforced as per published service requirements. Conditional documents: DOC-01 (as per conditions). | Must | System blocks submission when mandatory conditions are not met. |
| BR-04 | DOC-01 (payment receipt) shall be required only when payment details are not updated in the system. | Must | System prompts and enforces receipt upload when transaction posting is missing/unavailable. |
| BR-05 | Where service page indicates physical verification is required, verification status must be captured before final approval. | Should | Final approving officer cannot approve unless required verification is marked completed (configurable). |
...
| IR-06 | Integration with Accounts/Ledger system to verify dues and payment reconciliation (Open Question). | Should | System can determine dues status and payment postings for a property. |
| IR-07 | Integration with Payment Gateway/Receipt repository if online payment is used (Open Question). | Could | Payment status is received and stored; receipt can be fetched. |
| IR-08 | Integration with digital signature/eSign service for digitally signed No Due Certificate (Recommendation). | Should | Issued certificate can be digitally signed and validated. |
...
| Approved | Applicant | SMS/Email/In-app | Approval notification and link to download No Due Certificate (NDC) output. |
...
| REP-07 | Issuance register \- No Due Certificate (NDC) outputs issued | Should | Register contains ARN, property, applicant, issue date, and output reference. |

# **17\. SLAs/SLS, Escalation & Rework Rules**

## **17.1 Published SLS**

As per the portal, overall application disposal time is 5 days with stage-wise allocation: Clerk 1 days, Sr. Assistant (Accounts) 3 days, Account Officer 1 days.
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| TC-01 | Happy path \- application approved within SLS | Citizen logged in; valid property identifiers available; all mandatory documents available. | Submit application with all mandatory fields and documents; each stage forwards; final authority approves. | Application is approved; No Due Certificate (NDC) output is generated and downloadable; SLA shows within 5 days. | Happy Path |
...
| TC-06 | Receipt required only when payment details are not updated | System does not show payment posting for the property (assumption). | Attempt to submit without receipt; then upload receipt and resubmit. | First submission attempt blocked; second succeeds with receipt. | Conditional Document |
...
| 2 | System \- Validate mandatory fields and required document presence; create workflow task for Clerk; send acknowledgement | FR-01, FR-02, FR-03, FR-04, FR-05, FR-06, FR-07, FR-11, FR-14, FR-15, FR-16, FR-17, BR-01, BR-02, BR-07, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 3 | Clerk \- Initial scrutiny; verify request and property identifiers; forward to Accounts scrutiny or raise query/reject (assumed) | FR-07, FR-08, BR-01, BR-02, BR-07, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 4 | Applicant \- Provide clarifications/corrections; re-upload document(s) if required; resubmit | FR-02, FR-08, FR-09, FR-10, BR-01, BR-02, BR-07, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 5 | Sr. Assistant (Accounts) \- Verify ledger/dues status; validate payment details; review receipt if required; forward to Account Officer or raise query (assumed) | FR-07, FR-09, BR-01, BR-02, BR-07, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 6 | Account Officer \- Final approval/rejection for issuance of No Due Certificate; record remarks | FR-07, FR-10, BR-01, BR-02, BR-07, DR-06, DR-07, NFR-01, NFR-03, NFR-08, FR-12, FR-13, FR-14, FR-15, BR-10, BR-11 | TC-01, TC-07, TC-12 |
| 7 | System \- Generate No Due Certificate (NDC) output on approval and/or rejection order; close application; notify applicant | FR-01, FR-02, FR-03, FR-04, FR-05, FR-06, FR-07, FR-11, FR-14, FR-15, FR-16, FR-17, BR-01, BR-02, BR-07, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 8 | Applicant \- Download output and view final status | FR-07, FR-12, REP-01, REP-02, REP-03, REP-04, REP-05, REP-06, REP-07 | TC-01 |
...
* Output includes an approval/rejection order and downloadable No Due Certificate (NDC) output where applicable; exact format and signing requirements to be confirmed.  
...
10. Accounts integration: Which ledger system is authoritative for 'dues' and 'receipt posted'? What is the reconciliation process when receipt is uploaded?

## **19.6 References**

1\. PUDA portal \- Service Details: 'Issue of No Due Certificate' (Service ID 23). Source document captured as PDF from puda.gov.in (accessed 02 Feb 2026).
