...
Service: Issuance of Conveyance Deed
...
Document Title: BRD \- Issuance of Conveyance Deed

Owning Department: Estate/Property Branch (Assumption \- confirm with PUDA)

Primary Source of Truth: PUDA portal 'Issuance of Conveyance Deed' service details (Service ID 22), accessed 02 Feb 2026\.

Application Disposal Time (SLS): 15 Days (as published on service details page).
...
The 'Issuance of Conveyance Deed' service enables a citizen/applicant to apply online for issuance of Conveyance Deed related outputs for a property managed by the authority. The digital workflow supports role-based scrutiny, document submission and verification, time-bound processing as per published Service Level Standards (SLS), and transparent status tracking for the applicant.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Issuance of Conveyance Deed |
| Service Category | Property/Plot Services |
| Authorities | PUDA, GMADA, GLADA, BDA (as displayed on portal header) |
| Portal Service URL | https://www.puda.gov.in/service-details/22 |
| Application Disposal Time (SLS) | 15 Days |
| Workflow Roles | Submit Application \-\> Clerk (\~4 Days) \-\> Senior Assistant (\~5 Days) \-\> Assistant Estate Officer (\~3 Days) \-\> Estate Officer (\~3 Days) \-\> Disposal |
| Document(s) Required | Self-attested copy of No Due Certificate (Not required if No Due Certificate issued online); Certified copy of GPA / Sub Attorney (if applicable); Copies of Conveyance Deed (CD) including one on stamp paper (stamp duty as applicable on date of execution; check with Sub Registrar SAS Nagar) |
| Physical Verification | Required (as indicated for each required document on the service page). |
| Output | Conveyance Deed issuance decision and (on approval) issuance of Conveyance Deed document to applicant (format/signing to be confirmed). |
...
Request by applicant for Conveyance Deed related service for a property in authority records.
...
\- No Due Certificate is available or verifiable online (as applicable).

## **3.3 Postconditions**

\- On approval, Conveyance Deed output is issued and made available for applicant download (Assumption \- format/signing to be confirmed).  
...
| :---- | :---- | :---- | :---- |
| Clerk | Initial scrutiny of application and uploaded documents; verify completeness; record remarks; forward/raise query/reject (query/reject assumed). | Internal portal dashboard | 4 Days |
| Senior Assistant | Secondary scrutiny; verify No Due Certificate (if applicable), GPA/sub-attorney (if any), and conveyance deed draft/copies; forward/raise query/reject (assumed). | Internal portal dashboard | 5 Days |
| Assistant Estate Officer | Tertiary review; confirm document and physical verification completion; ensure eligibility/compliance; forward to Estate Officer or raise query (assumed). | Internal portal dashboard | 3 Days |
| Estate Officer | Final decision; approve or reject; record final remarks; dispose the application. | Internal portal dashboard | 3 Days |
...
\- Citizen-facing online application form for Issuance of Conveyance Deed including required fields and document upload.  
...
\- Stamp duty payment and Sub-Registrar processes (informational guidance only).
...
| GPA | General Power of Attorney (authorisation document). |
| CD | Conveyance Deed. |
...
The digital process will allow online submission and tracking, automated task routing to internal roles, document management, verification/inspection capture (where applicable), and disposal within 15 days as published. The workflow will support structured decisions (forward/query/approve/reject) and time-bound processing.

# **9\. Workflow Details**

The published processing sequence is: Submit Application \-\> Clerk (\~4 Days) \-\> Senior Assistant (\~5 Days) \-\> Assistant Estate Officer (\~3 Days) \-\> Estate Officer (\~3 Days) \-\> Disposal.
...
| 3 | Clerk | Application \+ uploaded documents | Initial scrutiny of application and uploaded documents; verify completeness; record remarks; forward/raise query/reject (query/reject assumed). | Forwarded / Query raised / Rejected | Pending at Senior Assistant OR Query Pending OR Rejected | 4 Days | Required (as indicated for each required document on the service page). |
...
| 5 | Senior Assistant | Application package \+ prior stage remarks | Secondary scrutiny; verify No Due Certificate (if applicable), GPA/sub-attorney (if any), and conveyance deed draft/copies; forward/raise query/reject (assumed). | Forwarded / Query raised / Rejected | Pending at Assistant Estate Officer OR Query Pending OR Rejected | 5 Days | If query is raised, application returns after resubmission (configurable). |
| 6 | Assistant Estate Officer | Application package \+ prior stage remarks | Tertiary review; confirm document and physical verification completion; ensure eligibility/compliance; forward to Estate Officer or raise query (assumed). | Forwarded / Query raised / Rejected | Pending at Estate Officer OR Query Pending OR Rejected | 3 Days | If query is raised, application returns after resubmission (configurable). |
| 7 | Estate Officer | Application package \+ verification records | Final decision; approve or reject; record final remarks; dispose the application. Approve or reject with mandatory remarks. | Approved / Rejected | Approved OR Rejected | 3 Days | Final approval may be contingent on completion of physical verification/inspection (as applicable). |
| 8 | System | Final decision | Generate Conveyance Deed output on approval and/or rejection order; close application; notify applicant. | Output generated; application closed. | Closed | Same day | Digital signing/QR and output format to be confirmed; treated as recommendation if implemented. |
| 9 | Applicant | Disposed application | Download output and view final status. | Download/print as needed. | Closed | NA | If physical pickup/original submission is required, portal must display instructions (Open Question). |
...
| Pending at Senior Assistant | Application pending with Senior Assistant for review. | Forwarded to Senior Assistant | Senior Assistant forwards/query/reject | Pending at Senior Assistant \-\> Query Pending / Pending at Assistant Estate Officer / Rejected |
| Pending at Assistant Estate Officer | Application pending with Assistant Estate Officer for review. | Forwarded to Assistant Estate Officer | Assistant Estate Officer forwards/query/reject | Pending at Assistant Estate Officer \-\> Query Pending / Pending at Estate Officer / Rejected |
| Pending at Estate Officer | Application pending with Estate Officer for final decision. | Forwarded to Estate Officer | Estate Officer approves/rejects | Pending at Estate Officer \-\> Approved / Rejected |
...
The system shall maintain the following key entities for the Issuance of Conveyance Deed service:
...
| Property | UPN, Area, Authority Name, Plot Number, Property Type, Scheme Name. Property master linkage via UPN is assumed. |
| Payment/Receipt | Payment reference(s), receipt upload, reconciliation status (to be confirmed). |
...
| Application Form | Applicant Details | Full Name, Remark | Citizen (editable before submit); read-only after submit |
| Application Form | Property Details | UPN, Area, Authority Name, Plot Number, Property Type, Scheme Name | Citizen enters UPN/Plot; system may auto-fill other fields (Assumption) |
| Application Form | Document Upload | DOC-01: Self-attested copy of No Due Certificate (Not required if No Due Certificate issued online); DOC-02: Certified copy of GPA / Sub Attorney (if applicable); DOC-03: Copies of Conveyance Deed (CD) including one on stamp paper (stamp duty as applicable on date of execution; check with Sub Registrar SAS Nagar) | Citizen uploads; internal can mark verified and add remarks |
...
| Submission | DOC-01 NDC | Required unless NDC is issued online and can be validated by system. | As per service page note; online NDC validation is assumption. |
| Submission | DOC-02 GPA/Sub Attorney | Required only if applicant acts through authorised representative. | Applicant should declare representation; rule to be confirmed. |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Self-attested copy of No Due Certificate (Not required if No Due Certificate issued online) | Applicant | Conditional | Submission | Physical verification required; if NDC is issued online, system should allow exemption (rule). |
| DOC-02 | Certified copy of GPA / Sub Attorney (if applicable) | Applicant | Conditional | Submission | Physical verification required; mandatory when application is filed through authorised representative. |
| DOC-03 | Copies of Conveyance Deed (CD) including one on stamp paper (stamp duty as applicable on date of execution; check with Sub Registrar SAS Nagar) | Applicant | Must | Submission | Physical verification required; portal to display guidance text from service page. |
...
| ID | Description | Priority | Acceptance Criteria |
| :---- | :---- | :---- | :---- |
| FR-01 | System shall allow citizens to select the Authority (PUDA/GMADA/GLADA/BDA) and initiate the 'Issuance of Conveyance Deed' service. | Must | Citizen can initiate the service for a selected authority. |
...
| FR-03 | System shall allow upload of required documents (DOC-01, DOC-02, DOC-03) and store them securely with metadata and versioning. | Must | Upload succeeds for allowed formats; documents are retrievable by authorised users. |
| FR-04 | System shall enforce mandatory data/document validations before allowing final submission. | Must | Submission is blocked if mandatory fields or required documents are missing. |
| FR-05 | System shall generate an Application Reference Number (ARN) and acknowledgement receipt upon successful submission. | Must | ARN and acknowledgement are generated and visible/downloadable to applicant. |
| FR-06 | System shall provide application status tracking to citizens, including current stage, SLA status (optional), and decision history. | Must | Citizen can view status and timeline of actions. |
| FR-07 | System shall provide role-based internal dashboards for Clerk, Senior Assistant, Assistant Estate Officer, Estate Officer to view and process assigned cases. | Must | Internal users see only applications assigned to their role/queue as per RBAC. |
| FR-08 | 'Clerk' role user shall be able to review the application, record scrutiny remarks, and take action: Forward to next stage, Raise Query to Applicant, or Reject (Query/Reject are assumptions unless confirmed). | Should | Role user can perform actions and system changes status accordingly; actions are logged. |
| FR-09 | 'Senior Assistant' role user shall be able to review the application, record scrutiny remarks, and take action: Forward to next stage, Raise Query to Applicant, or Reject (Query/Reject are assumptions unless confirmed). | Should | Role user can perform actions and system changes status accordingly; actions are logged. |
| FR-10 | 'Assistant Estate Officer' role user shall be able to review the application, record scrutiny remarks, and take action: Forward to next stage, Raise Query to Applicant, or Reject (Query/Reject are assumptions unless confirmed). | Should | Role user can perform actions and system changes status accordingly; actions are logged. |
| FR-11 | 'Estate Officer' role user shall be able to review the application, record scrutiny remarks, and take action: Forward to next stage, Raise Query to Applicant, or Reject (Query/Reject are assumptions unless confirmed). | Must | Role user can perform actions and system changes status accordingly; actions are logged. |
...
| FR-13 | On approval, system shall generate the Conveyance Deed output document and make it available for applicant download (Assumption). | Should | Downloadable output is generated and accessible to applicant and authorised staff. |
...
| FR-15 | System shall provide configurable SLA timers per stage (as per published durations) and calculate overall service disposal time (15 days). | Must | SLA clocks and breach flags are visible in dashboards and reports. |
...
| :---- | :---- | :---- | :---- |
| BR-01 | Overall service disposal time shall be 15 days (as published on service details page). | Must | System SLA calculation shows total processing within published SLS for non-query cases. |
| BR-02 | Stage-wise SLS shall be as published: Clerk 4 days, Senior Assistant 5 days, Assistant Estate Officer 3 days, Estate Officer 3 days. | Must | SLA timer per stage is configured to published values and breach is flagged. |
| BR-03 | Document submission rules shall be enforced as per published service requirements. Mandatory documents: DOC-03. Conditional documents: DOC-01, DOC-02 (as per conditions). | Must | System blocks submission when mandatory conditions are not met. |
| BR-04 | DOC-01 (No Due Certificate) shall not be required if the No Due Certificate was issued online and is verifiable in the system (Assumption). | Should | System allows submission without DOC-01 when online NDC reference exists; otherwise requires upload. |
| BR-05 | DOC-02 (GPA/Sub Attorney) shall be mandatory when applicant applies through an authorised representative. | Must | Submission is blocked when representation is declared but DOC-02 is missing. |
| BR-06 | Where service page indicates physical verification is required, verification status must be captured before final approval. | Must | Final approving officer cannot approve unless required verification is marked completed (configurable). |
| BR-07 | Rejection shall require mandatory reason/remarks captured by the rejecting officer. | Must | Reject action cannot be completed without reason/remarks. |
| BR-08 | Only the assigned role/stage owner shall be able to process an application at that stage. | Must | RBAC prevents unauthorised actions; audit shows blocked attempts. |
| BR-09 | When a query is raised, the application shall move to 'Query Pending' and return to the same stage upon resubmission. | Should | Query/resubmission loop works and stage is preserved. |
| BR-10 | SLA clock treatment during query/resubmission (pause vs reset) shall be configurable. | Should | Configuration exists and reports reflect chosen SLA policy. |
| BR-11 | If any fee/payment is applicable for this service (not provided in input), processing shall start only after successful payment. | Could | Applications with payment pending are not routed to processing stages; they move after payment success. |
| BR-12 | All decisions and actions shall be time-stamped and auditable. | Must | Audit log contains timestamps for each action. |
...
| IR-06 | Integration with No Due Certificate records to validate whether NDC is issued online (Assumption). | Could | System can fetch NDC status and exempt DOC-01 where applicable. |
| IR-07 | Integration with digital signature/eSign service for issuing digitally signed conveyance deed / orders (Recommendation). | Should | Generated outputs can be digitally signed and validated. |
...
| Approved | Applicant | SMS/Email/In-app | Approval notification and link to download Conveyance Deed output. |
...
| REP-07 | Issuance register \- Conveyance Deed outputs issued | Should | Register contains ARN, property, applicant, issue date, and output reference. |

# **17\. SLAs/SLS, Escalation & Rework Rules**

## **17.1 Published SLS**

As per the portal, overall application disposal time is 15 days with stage-wise allocation: Clerk 4 days, Senior Assistant 5 days, Assistant Estate Officer 3 days, Estate Officer 3 days.
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| TC-01 | Happy path \- application approved within SLS | Citizen logged in; valid property identifiers available; all mandatory documents available. | Submit application with all mandatory fields and documents; each stage forwards; final authority approves. | Application is approved; Conveyance Deed output is generated and downloadable; SLA shows within 15 days. | Happy Path |
...
| TC-06 | NDC not required when issued online (conditional doc) | Citizen has property with NDC already issued online (assumption). | Initiate service; system detects online NDC; citizen submits without uploading DOC-01. | Submission succeeds; system records NDC reference; application moves forward. | Conditional Document |
...
| 2 | System \- Validate mandatory fields and required document presence; create workflow task for Clerk; send acknowledgement | FR-01, FR-02, FR-03, FR-04, FR-05, FR-06, FR-07, FR-12, FR-15, FR-16, FR-17, FR-18, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 3 | Clerk \- Initial scrutiny of application and uploaded documents; verify completeness; record remarks; forward/raise query/reject (query/reject assumed) | FR-07, FR-08, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 4 | Applicant \- Provide clarifications/corrections; re-upload document(s) if required; resubmit | FR-02, FR-08, FR-09, FR-10, FR-11, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 5 | Senior Assistant \- Secondary scrutiny; verify No Due Certificate (if applicable), GPA/sub-attorney (if any), and conveyance deed draft/copies; forward/raise query/reject (assumed) | FR-07, FR-09, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 6 | Assistant Estate Officer \- Tertiary review; confirm document and physical verification completion; ensure eligibility/compliance; forward to Estate Officer or raise query (assumed) | FR-07, FR-10, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 7 | Estate Officer \- Final decision; approve or reject; record final remarks; dispose the application | FR-07, FR-10, FR-11, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08, FR-13, FR-14, FR-15, FR-16, BR-11, BR-12 | TC-01, TC-07, TC-12 |
| 8 | System \- Generate Conveyance Deed output on approval and/or rejection order; close application; notify applicant | FR-01, FR-02, FR-03, FR-04, FR-05, FR-06, FR-07, FR-12, FR-15, FR-16, FR-17, FR-18, BR-01, BR-02, BR-08, DR-06, DR-07, NFR-01, NFR-03, NFR-08 | TC-01, TC-05, TC-08 |
| 9 | Applicant \- Download output and view final status | FR-07, FR-13, REP-01, REP-02, REP-03, REP-04, REP-05, REP-06, REP-07 | TC-01 |
...
* Output includes an approval/rejection order and downloadable Conveyance Deed output where applicable; exact format and signing requirements to be confirmed.  
...
* Online verification of No Due Certificate status is possible if NDC was issued online (integration to be confirmed).
...
10. Stamp duty guidance: Should the portal capture stamp duty payment details or only display guidance to check with Sub Registrar?

## **19.6 References**

1\. PUDA portal \- Service Details: 'Issuance of Conveyance Deed' (Service ID 22). Source document captured as PDF from puda.gov.in (accessed 02 Feb 2026).
