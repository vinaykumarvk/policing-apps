...
**Service: Registration of Architect**
...
Document Title: BRD \- Registration of Architect

Owning Department: Town Planning / Building Plan Branch (Assumption \- confirm with PUDA)

Primary Source of Truth: PUDA portal 'Registration of Architect' service details (https://www.puda.gov.in/service-details-empanellment/1710)

Application Disposal Time (SLS): 4 Days
...
The 'Registration of Architect' service enables an applicant to submit an online request via the PUDA Citizen Services Portal/Mobile App. The system shall capture application details and required documents, route the request through role-based scrutiny/verification stages, enforce Service Level Standards (SLS), and on disposal generate a downloadable output (letter/certificate/order) along with notifications and audit trails.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Registration of Architect |
| Service Category | Empanelment / Professional Registration |
| Authorities | PUDA, GMADA, GLADA, BDA (as displayed on portal header) |
| Application Disposal Time (SLS) | 4 Days |
| Workflow Roles | Clerk (2 days) \-\> Draftsman (2 days) \-\> Disposal |
| Document(s) Required | Upload Valid Council of Architecture Certificate; Education Qualification Certificate B.Tech/B.E Architecture; Address Proof (Upload Passport/Voter id/Driving Licence); ... |
| Physical Verification | Not indicated on service page.  |
| Output | Architect Registration / Empanelment Certificate (downloadable). |
...
* Architect seeks registration/empanelment with the authority to submit building plans or related applications (exact usage to be confirmed).

Preconditions (to be validated):

* Architect possesses a valid Council of Architecture registration certificate.  
* Architect can provide PAN, Aadhaar, qualification proof and address proofs as required.
...
| 5 | Draftsman | Application package \+ previous stage remarks/verification records | Final review; confirm verification; approve or reject; record final remarks | Approved / Rejected | Approved OR Rejected | 2 Days | If physical verification is required, completion may be mandatory prior to approval (Assumption). |
| 6 | System | Final decision and approval remarks | Generate certificate/letter (approval or rejection order), close application, and notify applicant | Output generated; application closed | Closed | Same day | Digital signature/QR code on output \- recommendation; to be confirmed. |
| 7 | Applicant | Disposed application | Download output and view final status | Download/print as needed | Closed | NA | If physical submission of originals is required, it must be communicated (Open Question). |
...
| Pending at Clerk | Application is under initial scrutiny. | System assignment to role | Role forwards / raises query / rejects | \-\> Pending at Draftsman OR \-\> Query Pending OR \-\> Rejected |
...
| Pending at Draftsman | Application under secondary/technical verification. | Forward from previous role | Role forwards / raises query / rejects / approves | \-\> Approved/Rejected OR \-\> Query Pending OR \-\> Pending at next role |
...
| :---- | :---- | :---- | :---- |
| 1 | Application Details | Select Authority; PAN of Architect | Citizen-provided. Editable in Draft; locked after submit unless query. |
| 2 | Personal Details | Salutation; Full Name; Father's Name; Gender; Marital Status; Date of Birth; UID/Aadhaar Number; Email Address; Mobile Number | Citizen-provided. Editable in Draft; locked after submit unless query. |
| 3 | Council of Architecture (CoA) Registration Details | Certificate Number; Valid From; Valid Till | Citizen-provided. CoA fields to be validated (manual or online \- to be confirmed). |
| 4 | Addresses | Permanent Address (Full Address, State, District, Pincode); Official Address (Same as Permanent?; Full Address, State, District, Pincode) | Citizen-provided. Editable in Draft; locked after submit unless query. |
| 5 | Documents & Submit | Upload mandatory documents as per Section 10 | Architect uploads and submits. After submit, only resubmission possible via query. |
...
| DR-02 | System shall capture core application fields as per the published form (Select Authority, PAN, Personal Details, CoA certificate details, Addresses). | Must | All published core fields are present on the form and persisted with the application. |
...
| DR-04 | System shall capture and store service-specific details (Architect registration details (certificate no., validity)) as structured data. | Must | Service-specific fields are stored and visible to processing officers; exports include these fields where relevant. |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Upload Valid Council of Architecture Certificate | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |
| DOC-02 | Education Qualification Certificate B.Tech/B.E Architecture | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |
| DOC-03 | Address Proof (Upload Passport/Voter id/Driving Licence) | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |
| DOC-04 | Aadhar Card | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |
| DOC-05 | Pan Card | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |

# **11\. Functional Requirements (FR)**

| ID | Description | Priority | Acceptance Criteria |
| :---- | :---- | :---- | :---- |
| FR-01 | System shall allow users to select the Authority (PUDA/GMADA/GLADA/BDA) and initiate the 'Registration of Architect' service. | Must | User can initiate the service for a selected authority; service landing page shows key requirements and SLS. |
...
| FR-11 | System shall support service-specific verification needs: CoA certificate validity (Valid From/Valid Till) must be checked before approval (manual or integration \- to be confirmed). | Should | Verification outcome is captured as structured fields/remarks; approval can be configured to require verification completion. |
| FR-12 | System shall support fee/payment capture for the service where applicable. Fee is not specified on the service details page; to be confirmed. | Should | If fee is configured: payment is captured (online transaction or offline instrument details) and receipt/reference is stored; failures are handled gracefully. |
...
| BR-02 | PAN of Architect shall be mandatory. | Must | Submission is blocked if PAN is missing or invalid format. |
| BR-03 | Council of Architecture certificate must be valid on the date of application (Valid Till not expired). | Must | If Valid Till \< submission date, system blocks submission or flags for rejection (policy to be confirmed). |
...
| IR-05 | System should support verification of Council of Architecture registration (manual upload validation or external verification if available). | Could | If integration is available, system can validate certificate number and validity; otherwise manual verification is recorded. |
...
| :---- | :---- | :---- | :---- |
| Application Submitted | Applicant | SMS/Email/In-app | Acknowledgement with ARN for Registration of Architect. |
...
PUDA: Your 'Registration of Architect' application has been submitted. ARN: {ARN}. Track status on portal.

SMS \- Query Raised (Template)

PUDA: Query raised for ARN {ARN} in 'Registration of Architect'. Please respond by {DUE\_DATE} on portal.

SMS \- Application Approved (Template)

PUDA: Your 'Registration of Architect' application ARN {ARN} is approved. Download certificate/letter from portal.
...
As per the service details page, overall application disposal time is 4 days with stage-wise allocation:
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| TC-ARCH-01 | Happy path approval without query | Applicant logged in; required data and documents available. | Fill form \-\> upload docs \-\> submit \-\> internal processing \-\> approve \-\> download output | Application is approved within SLS; output is generated and downloadable. | Happy |
| TC-ARCH-02 | Mandatory field validation | Applicant logged in. | Attempt submission with missing mandatory fields | System blocks submission and highlights missing fields. | Negative |
| TC-ARCH-03 | Mandatory document missing | Applicant logged in. | Attempt submission without uploading a mandatory document | System blocks submission and shows required document checklist. | Negative |
| TC-ARCH-04 | Query raised and resubmission | Application submitted successfully. | Officer raises query \-\> applicant receives notification \-\> applicant edits allowed fields/re-uploads docs \-\> resubmits | Application returns to the originating officer; query is marked resolved on further processing. | Rework |
| TC-ARCH-05 | Rejection by competent authority | Application submitted successfully. | Application processed \-\> approver rejects with reason code and remarks | Status becomes Rejected; rejection order is generated; applicant is notified. | Negative |
| TC-ARCH-06 | SLA breach at an internal stage | Application pending at a stage beyond due date. | Let task exceed configured due date | Task flagged overdue; escalation notification sent; breach recorded in report. | SLA |
| TC-ARCH-07 | Document mismatch detected during scrutiny | Application submitted. | Officer marks document mismatch and raises query or rejects | System captures mismatch reason; applicant is notified; workflow follows decision. | Negative |
| TC-ARCH-08 | Certificate validity expired (service-specific) | Applicant enters expired validity date (where applicable). | Submit with expired/invalid validity data | System blocks submission or flags for rejection as per rule configuration. | Negative |
| TC-ARCH-09 | No-fee service does not prompt for payment | Applicant initiates service. | Proceed to submission flow | No payment step is shown by default; submission succeeds based on data/docs. | Functional |
| TC-ARCH-10 | Duplicate application detection | An existing active application exists for same key identifiers. | Submit a second application with same identifiers | System warns or blocks duplicate submission as per configured rule. | Negative |
| TC-ARCH-11 | Role-based access control for internal staff | Two staff users with different roles. | Attempt to access applications outside role/jurisdiction | Access denied or masked data; attempt is logged. | Security |
| TC-ARCH-12 | Output download and verification | Application disposed. | Applicant downloads output; verify QR/digital signature (if enabled) | Output is readable and verifiable; download logged in audit. | Functional |
...
| :---- | :---- | :---- | :---- |
| 1 | Fill application form, upload documents and submit application | FR-01, FR-02, FR-03, FR-04, FR-05, DR-01, DR-02, DR-03 | TC-ARCH-01, TC-ARCH-02, TC-ARCH-03 |
| 2 | Validate mandatory fields and required documents | FR-06, FR-08, FR-14, FR-15, DR-01, DR-06, NFR-09 | TC-ARCH-01, TC-ARCH-12 |
| 3 | Initial scrutiny | FR-08, FR-09, FR-10, FR-13, BR-06, DR-06, DR-08 | TC-ARCH-04, TC-ARCH-05, TC-ARCH-06 |
| 4 | Provide clarifications/corrections | FR-01, FR-02, FR-03, FR-04, FR-05, DR-01, DR-02, DR-03 | TC-ARCH-01, TC-ARCH-02, TC-ARCH-03 |
| 5 | Final review | FR-08, FR-09, FR-10, FR-13, BR-06, DR-06, DR-08 | TC-ARCH-04, TC-ARCH-05, TC-ARCH-06 |
| 6 | Generate certificate/letter (approval or rejection order), close application, and notify applicant | FR-06, FR-08, FR-14, FR-15, DR-01, DR-06, NFR-09 | TC-ARCH-01, TC-ARCH-12 |
| 7 | Download output and view final status | FR-01, FR-02, FR-03, FR-04, FR-05, DR-01, DR-02, DR-03 | TC-ARCH-01, TC-ARCH-02, TC-ARCH-03 |

## **19.2 Dependencies**

| Dependency | Description | Impact if Not Available |
| :---- | :---- | :---- |
| Professional Registry / CoA Verification | Ability to verify Council of Architecture registration details (manual or integration). | Higher risk of invalid registrations; longer verification time. |
...
* What is the validity period of the registration and is renewal supported via the same service?  
* Is Council of Architecture (CoA) verification required through any online integration, or manual verification based on uploaded certificate?  
* What are the rejection reason codes to be published to applicants?  
...
* Service details page (input): https://www.puda.gov.in/service-details-empanellment/1710  
