...
**Service: Issuance of Temporary Sewerage Connection (For Construction Purpose)**
...
**Document Title:** BRD \- Issuance of Temporary Sewerage Connection (For Construction Purpose)

**Owning Department:** Engineering / Water Supply & Sewerage Branch (Assumption \- confirm with PUDA)

**Primary Source of Truth:** PUDA portal service details (https://www.puda.gov.in/service-details/1059), accessed 03 Feb 2026\.

**Application Disposal Time (SLS):** TBD (not available in input).
...
The 'Issuance of Temporary Sewerage Connection (For Construction Purpose)' service enables an applicant to submit an online request via the PUDA citizen portal for temporary sewerage connection permission/sanction letter (digital output); connection reference id. Published SLS and workflow stage timelines are not available in the provided input for this service (Open Question). The service details page indicates that online application submission will be available soon; portal behaviour for availability must be confirmed.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Issuance of Temporary Sewerage Connection (For Construction Purpose) |
...
| Application Disposal Time (SLS) | TBD |
| Workflow Roles | TBD (workflow roles not available in input). |
| Document(s) Required | Copy of Building Plan; Certificate from a plumber registered with the Authority; Copy of approved building plans letter issued by architect; ... |
| Physical Verification | Not indicated on service page |
| Output | Temporary sewerage connection permission/sanction letter (digital output); connection reference ID. |
...
Applicant requires a temporary sewerage connection for construction purpose for a PUDA/ADA property.
...
* Approved building plan and plumber/architect documents are available for construction purpose.
...
Internal roles are not available in input for this service. Configure and confirm roles and routing (Open Question).
...
The published processing sequence (roles/timelines) is not available in input; workflow must be configured by PUDA (Open Question).
...
| 2 | System | Submitted application | Validate mandatory fields and required document presence; create workflow task for first role; send acknowledgement | Task assigned to TBD role; acknowledgement to applicant | Pending at TBD | Immediate | Auto-validation rules (e.g., UPN lookup) \- to be confirmed. |
| 3 | System | Final decision | Generate digitally signed/approved output (order/permission/certificate as applicable); update records; close application; notify applicant | Output generated; application closed | Closed | Same day | Integration with property master, document signing and dispatch \- to be confirmed. |
| 4 | Applicant | Disposed application | Download/print output; view final status and history | Output downloaded | Closed | NA | If physical originals submission is required, it must be communicated to applicant (Open Question). |
...
| Submitted | Application submitted; system validations and assignment pending/complete. | Submit | Auto-assign | Submitted \-\> Pending at TBD |
...
| Resubmitted | Applicant resubmitted after query. | Resubmit | Assign back | Resubmitted \-\> Pending at TBD (same stage) |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Copy of Building Plan | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-02 | Certificate from a plumber registered with the Authority | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-03 | Copy of approved building plans letter issued by architect | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-04 | Estimate for construction work issued by Architect | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-05 | Undertaking regarding illegal construction / illegal water connection at site | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-06 | Self attested photographs of all owners | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-07 | Self attested Photo Id proofs of all owners | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-08 | Attested copy of GPA (if applicable) | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
...
| FR-11 | The system shall support optional physical verification capture if configured for the service. | Could | When enabled, verification record can be captured and required before approval. |
...
| BR-02 | Physical verification requirement (if any) shall be configurable per service and enforced accordingly. | Could | When enabled, verification status gating works as configured. |
...
Published application disposal time (SLS): TBD (not available in input).

| Stage/Role | Published Stage Time | Notes |
| :---- | :---- | :---- |
| TBD | TBD | Workflow stages not available in input; confirm roles and timelines. |
...
| Step 3 | System \- Generate digitally signed/approved outpu... | FR-01; FR-03; DR-01; NFR-03 | TC-01; TC-02; TC-03 |
| Step 4 | Applicant \- Download/print output; view final status... | FR-01; FR-03; DR-01; NFR-03 | TC-01; TC-02; TC-03 |
| Step 3 | System \- Output generation/closure | FR-12; IR-05; NFR-09; REP-01 | TC-01; TC-12 |
...
* Workflow roles and timelines are not provided in input; to be defined by PUDA.

## **19.5 Open Questions for Business Confirmation**

* Service page indicates online submission will be available soon \- confirm go-live readiness and interim portal behaviour (disable vs accept applications).  
...
PUDA Service Details Page: https://www.puda.gov.in/service-details/1059 (captured in input PDF).
