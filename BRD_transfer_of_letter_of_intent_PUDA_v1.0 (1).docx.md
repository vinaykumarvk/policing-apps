...
**Service: Transfer of Letter of Intent**
...
**Document Title:** BRD \- Transfer of Letter of Intent

**Owning Department:** Estate/Property Branch (Assumption \- confirm with PUDA)

**Primary Source of Truth:** PUDA portal service details (https://www.puda.gov.in/service-details/1048), accessed 03 Feb 2026\.

**Application Disposal Time (SLS):** 21 Days (as published on service details page).
...
The 'Transfer of Letter of Intent' service enables an applicant to submit an online request via the PUDA citizen portal for transfer/endorsement order for letter of intent (loi); updated loi holder details in records. The application is processed through a role-based workflow (Clerk (2 days) \-\> Senior Assistant (3 days) \-\> AEO (2 days) \-\> Estate Officer (3 days) \-\> Disposal). The published Service Level Standard (SLS) is 21 days. The service details page indicates that physical verification is required.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Transfer of Letter of Intent |
...
| Application Disposal Time (SLS) | 21 Days |
| Workflow Roles | Clerk (2 days) \-\> Senior Assistant (3 days) \-\> AEO (2 days) \-\> Estate Officer (3 days) \-\> Disposal |
| Document(s) Required | Affidavit from the Seller(s) that the Letter of Intent is free from all sorts of encumbrances or litigatio in any court of law, liens, mortgage etc.; Photo Identity proof of seller(s); Clearance against loan/ mortgage, if any; ... |
| Physical Verification | Required (as per portal note) |
| Output | Transfer/endorsement order for Letter of Intent (LOI); updated LOI holder details in records. |
...
Applicant seeks transfer of a Letter of Intent (LOI) from existing holder (seller) to purchaser/transferee.
...
* LOI is available (online LOI reference or scanned copy for manual LOI).
...
* AEO \- processes the application at assigned stage with role-specific permissions.  
...
The published processing sequence is: Submit Application \-\> Clerk (2 days) \-\> Senior Assistant (3 days) \-\> AEO (2 days) \-\> Estate Officer (3 days) \-\> Disposal.
...
| 3 | Clerk | Application package \+ remarks | Review application and documents; record remarks; if physical verification is required, initiate/record verification (assumption); forward to next role OR raise query OR reject | Forwarded / Query raised / Rejected | Pending at Senior Assistant OR Query Pending OR Rejected | 2 Days | Physical verification/original document checks may be mandatory before approval (confirm). |
...
| 5 | Senior Assistant | Application package \+ remarks | Review application and documents; record remarks; forward to next role OR raise query OR reject | Forwarded / Query raised / Rejected | Pending at AEO OR Query Pending OR Rejected | 3 Days | Physical verification/original document checks may be mandatory before approval (confirm). |
...
| 7 | AEO | Application package \+ remarks | Review application and documents; record remarks; forward to next role OR raise query OR reject | Forwarded / Query raised / Rejected | Pending at Estate Officer OR Query Pending OR Rejected | 2 Days | Physical verification/original document checks may be mandatory before approval (confirm). |
| 8 | Applicant (if query raised) | Query/deficiency memo from authority | Provide clarification; correct fields; re-upload document(s) if required; resubmit | Resubmitted application | Resubmitted to AEO | TBD | Query response time window and SLA pause/reset rules \- Open Question. |
| 9 | Estate Officer | Application package \+ remarks | Review application and documents; record remarks; approve/reject | Approved / Rejected | Approved OR Rejected | 3 Days | Physical verification/original document checks may be mandatory before approval (confirm). |
| 10 | System | Final decision | Generate digitally signed/approved output (order/permission/certificate as applicable); update records; close application; notify applicant | Output generated; application closed | Closed | Same day | Integration with property master, document signing and dispatch \- to be confirmed. |
| 11 | Applicant | Disposed application | Download/print output; view final status and history | Output downloaded | Closed | NA | If physical originals submission is required, it must be communicated to applicant (Open Question). |
...
| Pending at AEO | Application under processing by AEO. | Auto-assign/forward | Forward / Query / Reject | Forward to next role OR Query Pending OR Rejected |
...
| LOI Transfer Details | LOI reference, seller(s), purchaser(s), share to be transferred (%), appointment details (if manual LOI). |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Affidavit from the Seller(s) that the Letter of Intent is free from all sorts of encumbrances or litigatio in any court of law, liens, mortgage etc. | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-02 | Photo Identity proof of seller(s) | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-03 | Clearance against loan/ mortgage, if any | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-04 | Liability affidavit of Purchaser(s) / Doner(s) / Transferee(s) | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-05 | If LOI is issued manually upload scan image of original LOI and submit original LOI during appointment. (Not required if LOI is issued Online) | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-06 | Photo Identity proof of Purchaser(s) | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
| DOC-07 | Certified copy of GPA, if applicable | Applicant | Yes | Submission | File format/size to be configured; verify authenticity and legibility. |
...
| BR-06 | If LOI was issued manually, applicant shall upload scanned copy and submit original LOI during appointment as stated on service page. | Must | System captures LOI issue type and enforces upload/appointment requirement for manual LOI. |
...
Published application disposal time (SLS): 21 Days (as per service details page).

| Stage/Role | Published Stage Time | Notes |
| :---- | :---- | :---- |
| Clerk | 2 Days | Stage actions: forward/query/reject/approve as applicable. |
| Senior Assistant | 3 Days | Stage actions: forward/query/reject/approve as applicable. |
| AEO | 2 Days | Stage actions: forward/query/reject/approve as applicable. |
...
| Step 7 | AEO \- Review application and documents; record... | FR-01; FR-03; DR-01; NFR-03 | TC-01; TC-02; TC-03 |
| Step 8 | Applicant (if query raised) \- Provide clarification; correct fields; r... | FR-01; FR-03; DR-01; NFR-03 | TC-01; TC-02; TC-03 |
| Step 10 | System \- Output generation/closure | FR-12; IR-05; NFR-09; REP-01 | TC-01; TC-12 |
...
PUDA Service Details Page: https://www.puda.gov.in/service-details/1048 (captured in input PDF).
