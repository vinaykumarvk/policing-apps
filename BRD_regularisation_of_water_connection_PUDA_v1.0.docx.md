...
**Service: Regularisation of Water connection**
...
Document Title: BRD \- Regularisation of Water connection

Owning Department: Single Window / Water Supply Branch (Assumption \- confirm with PUDA)

Primary Source of Truth: PUDA portal 'Regularisation of Water connection' service details (https://www.puda.gov.in/service-details/1712)
...
The 'Regularisation of Water connection' service enables an applicant to submit an online request via the PUDA Citizen Services Portal/Mobile App. The system shall capture application details and required documents, route the request through role-based scrutiny/verification stages, enforce Service Level Standards (SLS), and on disposal generate a downloadable output (letter/certificate/order) along with notifications and audit trails.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Regularisation of Water connection |
| Service Category | Water & Sewerage Services |
| Authorities | PUDA, GMADA, GLADA, BDA (as displayed on portal header) |
| Application Disposal Time (SLS) | 7 Days |
| Workflow Roles | Clerk (2 days) \-\> Superintendent (Single window) (1 day) \-\> Disposal |
| Document(s) Required | Water connection Receipts; Copy of Sewerage connection letter (Not required if the Letter was issued online); Meter Guarantee Card copy |
| Physical Verification | Required. Details to be confirmed. |
| Output | Regularisation Approval / Updated Connection Status (downloadable letter if applicable). |
...
* Request to regularise an existing water connection/meter details for a property under the selected authority.

Preconditions (to be validated):

* Existing connection receipts and meter guarantee details are available.  
* If sewerage connection letter was issued offline, a copy is available; if issued online, system can waive upload as per checklist note.
...
| 3 | Clerk | Application \+ uploaded documents | Initial scrutiny; verify completeness and basic correctness; record remarks; forward OR raise query OR reject (query/reject assumed) | Forwarded / Query raised / Rejected | Pending at Superintendent (Single window) OR Query Pending OR Rejected | 2 Days | Physical verification requirement and responsibility to be confirmed (if applicable). |
...
| Pending at Clerk | Application is under initial scrutiny. | System assignment to role | Role forwards / raises query / rejects | \-\> Pending at Superintendent (Single window) OR \-\> Query Pending OR \-\> Rejected |
...
| Pending at Superintendent (Single window) | Application under secondary/technical verification. | Forward from previous role | Role forwards / raises query / rejects / approves | \-\> Approved/Rejected OR \-\> Query Pending OR \-\> Pending at next role |
...
| 3 | Water Meter Details | Water Meter No.; Make and Model; Installation Date; Water Meter Bill Number; Warranty in Years | Citizen-provided. Editable in Draft; locked after submit unless query. |
| 4 | Building / Usage Details | Whether Building Is (status \- To be confirmed); Number of Floors Constructed; Is Basement Constructed; Number of Seats for Ground Floor; Number of Seats for First Floor; Number of Seats for Second Floor | Citizen-provided. Editable in Draft; locked after submit unless query. |
| 5 | Documents & Submit | Upload mandatory documents as per Section 10 | Citizen uploads and submits. After submit, only resubmission possible via query. |
...
| DR-04 | System shall capture and store service-specific details (Water Meter Details; Building / Usage Details) as structured data. | Must | Service-specific fields are stored and visible to processing officers; exports include these fields where relevant. |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Water connection Receipts | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |
| DOC-02 | Copy of Sewerage connection letter (Not required if the Letter was issued online) | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |
| DOC-03 | Meter Guarantee Card copy | Citizen/Applicant | Yes | Submission | Attested/self-attested as applicable; validation by processing officer. |

# **11\. Functional Requirements (FR)**

| ID | Description | Priority | Acceptance Criteria |
| :---- | :---- | :---- | :---- |
| FR-01 | System shall allow users to select the Authority (PUDA/GMADA/GLADA/BDA) and initiate the 'Regularisation of Water connection' service. | Must | User can initiate the service for a selected authority; service landing page shows key requirements and SLS. |
...
| BR-02 | Copy of Sewerage connection letter is not required if the letter was issued online (as per checklist note). | Must | System allows user to mark 'Sewerage letter issued online' and waive the document requirement accordingly. |
| BR-03 | Water connection receipts and meter guarantee card copy shall be mandatory documents. | Must | Submission is blocked if required receipts/guarantee card are missing. |
...
| :---- | :---- | :---- | :---- |
| Application Submitted | Applicant | SMS/Email/In-app | Acknowledgement with ARN for Regularisation of Water connection. |
...
PUDA: Your 'Regularisation of Water connection' application has been submitted. ARN: {ARN}. Track status on portal.

SMS \- Query Raised (Template)

PUDA: Query raised for ARN {ARN} in 'Regularisation of Water connection'. Please respond by {DUE\_DATE} on portal.

SMS \- Application Approved (Template)

PUDA: Your 'Regularisation of Water connection' application ARN {ARN} is approved. Download certificate/letter from portal.
...
* Clerk: 2 day(s)  
* Superintendent (Single window): 1 day(s)
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| TC-RWC-01 | Happy path approval without query | Applicant logged in; required data and documents available. | Fill form \-\> upload docs \-\> submit \-\> internal processing \-\> approve \-\> download output | Application is approved within SLS; output is generated and downloadable. | Happy |
| TC-RWC-02 | Mandatory field validation | Applicant logged in. | Attempt submission with missing mandatory fields | System blocks submission and highlights missing fields. | Negative |
| TC-RWC-03 | Mandatory document missing | Applicant logged in. | Attempt submission without uploading a mandatory document | System blocks submission and shows required document checklist. | Negative |
| TC-RWC-04 | Query raised and resubmission | Application submitted successfully. | Officer raises query \-\> applicant receives notification \-\> applicant edits allowed fields/re-uploads docs \-\> resubmits | Application returns to the originating officer; query is marked resolved on further processing. | Rework |
| TC-RWC-05 | Rejection by competent authority | Application submitted successfully. | Application processed \-\> approver rejects with reason code and remarks | Status becomes Rejected; rejection order is generated; applicant is notified. | Negative |
| TC-RWC-06 | SLA breach at an internal stage | Application pending at a stage beyond due date. | Let task exceed configured due date | Task flagged overdue; escalation notification sent; breach recorded in report. | SLA |
| TC-RWC-07 | Document mismatch detected during scrutiny | Application submitted. | Officer marks document mismatch and raises query or rejects | System captures mismatch reason; applicant is notified; workflow follows decision. | Negative |
| TC-RWC-08 | Physical verification fail | Physical verification required for service. | Verifier records failed verification \-\> approver rejects (or raises query) | Decision recorded with verification remarks; applicant notified; output generated. | Negative |
| TC-RWC-09 | No-fee service does not prompt for payment | Applicant initiates service. | Proceed to submission flow | No payment step is shown by default; submission succeeds based on data/docs. | Functional |
| TC-RWC-10 | Duplicate application detection | An existing active application exists for same key identifiers. | Submit a second application with same identifiers | System warns or blocks duplicate submission as per configured rule. | Negative |
| TC-RWC-11 | Role-based access control for internal staff | Two staff users with different roles. | Attempt to access applications outside role/jurisdiction | Access denied or masked data; attempt is logged. | Security |
| TC-RWC-12 | Output download and verification | Application disposed. | Applicant downloads output; verify QR/digital signature (if enabled) | Output is readable and verifiable; download logged in audit. | Functional |
...
| :---- | :---- | :---- | :---- |
| 1 | Fill application form, upload documents and submit application | FR-01, FR-02, FR-03, FR-04, FR-05, DR-01, DR-02, DR-03 | TC-RWC-01, TC-RWC-02, TC-RWC-03 |
| 2 | Validate mandatory fields and required documents | FR-06, FR-08, FR-14, FR-15, DR-01, DR-06, NFR-09 | TC-RWC-01, TC-RWC-12 |
| 3 | Initial scrutiny | FR-08, FR-09, FR-10, FR-13, BR-06, DR-06, DR-08, FR-11, BR-05 | TC-RWC-04, TC-RWC-05, TC-RWC-06, TC-RWC-08 |
| 4 | Provide clarifications/corrections | FR-01, FR-02, FR-03, FR-04, FR-05, DR-01, DR-02, DR-03 | TC-RWC-01, TC-RWC-02, TC-RWC-03 |
| 5 | Final review | FR-08, FR-09, FR-10, FR-13, BR-06, DR-06, DR-08, FR-11, BR-05 | TC-RWC-04, TC-RWC-05, TC-RWC-06, TC-RWC-08 |
| 6 | Generate certificate/letter (approval or rejection order), close application, and notify applicant | FR-06, FR-08, FR-14, FR-15, DR-01, DR-06, NFR-09 | TC-RWC-01, TC-RWC-12 |
| 7 | Download output and view final status | FR-01, FR-02, FR-03, FR-04, FR-05, DR-01, DR-02, DR-03 | TC-RWC-01, TC-RWC-02, TC-RWC-03 |
...
* What is the definition of 'regularisation' in this context (e.g., regularising an existing connection, meter replacement, or late registration)?  
* Is any penalty/fee applicable based on time elapsed or illegal connection status?  
* Who performs physical verification if only Clerk/Superintendent roles are listed?  
...
* Service details page (input): https://www.puda.gov.in/service-details/1712  
