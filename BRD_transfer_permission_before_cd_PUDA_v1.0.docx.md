...
**Service: Transfer Permission (Before CD)**
...
Document Title: BRD \- Transfer Permission (Before CD)

Owning Department: Estate / Property Branch (Assumption)

Primary Source of Truth: PUDA service details page (input) accessed on 02 Feb 2026: https://www.puda.gov.in/service-details/21

Application Disposal Time (SLS): 10 Day(s) (as published on service details page).
...
The 'Transfer Permission (Before CD)' service enables citizens to apply online through the PUDA portal/mobile app, upload required documents, and obtain Transfer Permission Order (Before Conveyance Deed) after role-based scrutiny and approvals. The workflow supports tracking, queries/rework, and issuance of the final output.
...
| Attribute | Value |
| :---- | :---- |
| Service Name | Transfer Permission (Before CD) |
...
| Output | Transfer Permission Order (Before Conveyance Deed) |
| Physical Verification | Yes (as indicated on service page) |
| Fees/Payments | Not specified in input (Open Question). |
| Published Disposal Time (SLS) | 10 Day(s) |
| Source URL | https://www.puda.gov.in/service-details/21 |
...
* Original allotment/re-allotment letter is available for physical submission at the authority (as per service note).

## **3.3 Postconditions**

* Application is approved and Transfer Permission Order (Before Conveyance Deed) is issued to applicant, OR application is rejected with recorded reason.  
...
| :---- | :---- | :---- | :---- |
| Clerk | Scrutiny/verification and forwarding as per stage. | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 4 day(s) |
| Senior Assistant | Scrutiny/verification and forwarding as per stage. | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 4 day(s) |
| AEO | Estate branch scrutiny and recommendation (Assumption). | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 3 day(s) |
| Estate Officer | Final competent approval for high-value/large plot cases (Assumption). | View assigned applications; verify docs; record remarks; forward/query/reject per permissions. | 4 day(s) |
...
* Generation and delivery of Transfer Permission Order (Before Conveyance Deed) via portal/mobile download.  
...
* Digitally generated Transfer Permission Order (Before Conveyance Deed) published to applicant dashboard.
...
| 2 | Clerk | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- Clerk | 4 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 3 | Senior Assistant | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- Senior Assistant | 4 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 4 | AEO | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- AEO | 3 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 5 | Estate Officer | Application data; documents | Scrutinize application as per role responsibilities; verify documents; record remarks. | Approve to next stage / Raise query / Recommend rejection | Under Review \- Estate Officer | 4 day(s) | Query can be raised for missing/invalid documents; applicant resubmits. |
| 6 | System | Final approval decision | Generate Transfer Permission Order (Before Conveyance Deed) with unique number; apply digital signature (if enabled); store and publish to applicant dashboard | Transfer Permission Order (Before Conveyance Deed) issued / Application closed | Approved \- Issued / Rejected | Same day | If rejected, reason is communicated and application is closed. |
...
| :---- | :---- | :---- | :---- |
| Application Form | Applicant Details | Full Name Permission Type Relation | Citizen (editable until submission); read-only post submission |
| Application Form | Property Details | UPN Area Authority Name; Plot Number Property Type Scheme Name; Reserved Price Sale Type Usage Type | Citizen selects/enters; key attributes may be auto-fetched (assumption) |
| Application Form | Purchaser/Transferee Details | Purchaser Name Father Name Address; Mobile Email; Latest Passport size photo of Purchaser; Specimen Signature of Purchaser | Citizen provides; repeatable group |
| Application Form | Legal Heir Details | Legal Heir Of Legal Heir Name Relationship with Purchaser; Passport size photo of Legal Heir | Citizen provides if applicable; repeatable |
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| DOC-01 | Affidavit from the all Sellers that the property is free from all sorts of encumbrances or litigation in any court of law, liens, mortgage etc | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-02 | Liability affidavit of all Purchasers / Doners / Transferees | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-03 | Photo Identity Proof of All Sellers | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-04 | Original letter of Allotment / Re-allotment letter . Original to be submitted at Authority | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-05 | Clearance against loan / mortgage (if applicable) | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-06 | Self attested copy of sanction of Sewerage Connection / Occupation Certificate , if obtained | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
| DOC-07 | Photo Identity Proof of All Purchasers | Citizen/Applicant | Yes | Submission | Verify legibility, relevance to property, and authenticity as per internal SOP. |
...
| FR-18 | System shall allow applicant to capture multiple purchaser/transferee records and their identity/photographs as per the form. | Must | Applicant can add/edit/delete purchaser rows before submission; validations apply to each row. |
...
| BR-06 | Original allotment/re-allotment letter must be submitted physically at authority (as per service note); portal should capture acknowledgement number (Open Question). | Must | Rule is enforced via validations/workflow; evidence is visible in case history/audit. |
| BR-07 | At least one purchaser/transferee must be captured; identity proofs must be provided as per required documents. | Must | Rule is enforced via validations/workflow; evidence is visible in case history/audit. |
...
PUDA: Your 'Transfer Permission (Before CD)' application has been submitted. ARN: {ARN}. Track status on portal.

SMS \- Query Raised (Template)

PUDA: Query raised for ARN {ARN} in 'Transfer Permission (Before CD)'. Please respond by {DUE\_DATE} on portal.

SMS \- Application Approved (Template)

PUDA: Your 'Transfer Permission (Before CD)' application ARN {ARN} is approved. Download certificate/letter from portal.

SMS \- Application Rejected (Template)

PUDA: Your 'Transfer Permission (Before CD)' application ARN {ARN} is rejected. Reason: {SHORT\_REASON}. Details on portal.
...
As per the service details page, overall application disposal time is 10 day(s) with stage-wise allocation:

* Clerk: 4 day(s)  
* Senior Assistant: 4 day(s)  
* AEO: 3 day(s)  
* Estate Officer: 4 day(s)
...
* Does transfer permission require NOC/dues clearance check before approval? If yes, how is it validated in system?

## **19.6 References**

* Service details page (input): https://www.puda.gov.in/service-details/21  
